...
def _setup_gaussian_linear_inverse_problem(
    ...
    post_mean, post_covariance = (
        laplace_posterior_approximation_for_linear_models(
            Amatrix, prior_mean, prior_hessian, noise_covariance_inv, obs
        )
    )
    return (
        Amatrix,
...
class ExponentialQuarticLogLikelihoodModel(LogLikelihood):
    ...
    def _values(self, x: Array) -> Array:
        return -(0.1 * x[0] ** 4 + 0.5 * (2.0 * x[1] - x[0] ** 2) ** 2)[
            :, None
        ]
...
class TestMetropolis(unittest.TestCase):
    ...
    def _check_logpost_gradients(self, prior_variable):
        ...
        errors = check_gradients(
            mcmc_variable._log_bayes_numerator,
            True,
            test_sample,
            fd_eps=fd_eps,
        )
