...
class TestGaussianNetwork(unittest.TestCase):
    ...
    def test_conditional_probability_density_factorization(self):
        nvars1, nvars2 = [3, 2]
        Amat = np.random.normal(0.0, 1.0, (nvars2, nvars1))
        bvec = np.random.normal(0, 1, (nvars2))
        mean1 = np.random.normal(0, 1, nvars1)
        temp1 = np.random.normal(0.0, 1.0, (nvars1, nvars1))
        cov1 = temp1.T.dot(temp1)
        temp2g1 = np.random.normal(0.0, 1.0, (nvars2, nvars2))
    ...
    def test_hierarchical_graph_prior_same_nparams(self):
        ...
        cpd_mats = [
            None,
            cpd_scales[0] * np.eye(nparams[1], nparams[0]),
        ...
        for ii in range(1, nnodes):
            cpd_mean = np.ones((nparams[ii], 1)) * prior_means[ii] - cpd_mats[
                ii
            ].dot(np.ones((nparams[ii - 1], 1)) * prior_means[ii - 1])
            cpd_cov = prior_covs[ii] * np.eye(nparams[ii]) - cpd_mats[ii].dot(
                prior_covs[ii - 1] * np.eye(nparams[ii - 1])
            ).dot(cpd_mats[ii].T)
            graph.add_node(
                ii,
                label=node_labels[ii],
    ...
    def test_hierarchical_graph_prior_varying_nparams(self):
        ...
        assert np.allclose(
            prior[1],
            true_prior_cov[nparams[:-1].sum() :, nparams[:-1].sum() :],
        )
    ...
    def test_peer_graph_prior(self):
        ...
        cov = np.eye(nparams[ii]) * max(
            1e-8,
            prior_covs[ii]
            - np.dot(np.asarray(cpd_scales) ** 2, prior_covs[:ii]),
        )
        graph.add_node(
            ii,
            label=node_labels[ii],
            ...
            cpd_mat=cpd_mats[ii],
            cpd_mean=(
                prior_means[ii] - np.dot(cpd_scales[:ii], prior_means[:ii])
            )
        ...
        graph.add_edges_from(
            [
                (
                    ii,
                    nnodes - 1,
                    {"cpd_cov": np.eye(nparams[ii]) * cpd_scales[ii]},
                )
                for ii in range(nnodes - 1)
            ]
        )
        ...
        I1, I2, I3 = [np.eye(nparams[0])] * nnodes
        rows = [
            np.hstack([v1 * I1, 0 * I1, a31 * v1 * I1]),
    ...
    def test_one_node_inference(self):
        ...
        noise = [
            noise_std[ii]
            * np.random.normal(0, noise_std[ii], (nsamples[ii], 1))
        ]
        values_train = [
            b.dot(c) + s + n
            for b, c, s, n in zip(
        ...
        laplace = DenseMatrixLaplacePosteriorApproximation(
            data_cpd_mats[0],
            prior_means[ii] * np.ones((nparams[0], 1)),
            prior_covs[ii] * np.eye(nparams[0]),
            noise_covs[0],
            data_cpd_vecs[0],
        )
        laplace.compute(values_train[0])
        true_post = (laplace.posterior_mean(), laplace.posterior_covariance())
        assert np.allclose(gauss_post[1], true_post[1])
        assert np.allclose(gauss_post[0], true_post[0][:, 0])
