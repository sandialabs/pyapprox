...
class TestGAN:
    ...
    def test_conditional_gan(self):
        ...
        nqoi = 1
        Qmat = bkd.atleast2d(np.random.uniform(0, 1, (nqoi, nvars))+1)
        Qvec = bkd.atleast2d(np.random.uniform(0, 1, (nqoi, 1))+1)
        ...
        def generate_obs(samples):
            ...
            noise = bkd.atleast2d(
                np.random.normal(0, noise_std, vals.shape))
            return vals+noise
        ...
        priorpush_rv = stats.norm(priorpush_mean[0], np.sqrt(priorpush_cov[0, 0]))
        ...
        for ii in range(ndata_realizations):
            ...
            post_mean, post_cov = \
                laplace_posterior_approximation_for_linear_models(
                    bkd.to_numpy(Amat), mu,
                    prior_hessian, noise_cov_inv, obs)
            postpush_mean, postpush_cov = (
                push_forward_gaussian_though_linear_model(
                    bkd.to_numpy(Qmat), bkd.to_numpy(Qvec),
                    post_mean, post_cov))

            postpush_rv = stats.norm(
                postpush_mean[0, 0], np.sqrt(postpush_cov[0, 0]))
            print("Post Push. Mean", postpush_mean, fake_test_samples.mean(axis=1))
            print("Post Push. Covariance", postpush_cov,
            ...
            ax.plot(xx[0], postpush_rv.pdf(xx[0]), ls='-', label=f"Post PDF {ii}",
                    lw=3)
            gen_kde = stats.gaussian_kde(fake_test_samples)
            # ax.hist(fake_test_samples[0], bins=30, density=True,
            #     label='Fake samples')
            ax.plot(xx[0], gen_kde(xx), ls='--', label=f"Fake Post PDF {ii}",
                    lw=3)
        ax.legend()
