...
def solve_SSD_constrained_least_squares(
    ...
    g = gradient(lstsq_coef, num_samples,
                 num_basis_terms, eta_indices.shape[0])
...
class SLSQPDisutilitySSDOptProblem(object):
    def __init__(self, basis_matrix, values, eta, probabilities):
        ...
        for ii in range(self.nnl_constraints):
            ...
            self.init_guess[lb:ub] = np.maximum(
                0, self.values-approx_values[ii])
    ...
    def solve(self, optim_options=None):
        ...
        def ineq_cons_fun(x): return self.nonlinear_constraints(x)
        def ineq_cons_jac(x): return self.nonlinear_constraints_jacobian(x)
        ineq_cons1 = {'type': 'ineq', 'fun': ineq_cons_fun,
                      'jac': ineq_cons_jac}

        def ineq_cons_fun(x): return (
            self.linear_constraint_matrix.dot(x)-self.linear_constraint_vector)

        def ineq_cons_jac(x): return self.linear_constraint_matrix.todense()
...
class SmoothDisutilitySSDOptProblem(TrustRegionDisutilitySSDOptProblem):
    ...
    def nonlinear_constraints_jacobian(self, x):
        ...
        for ii in range(self.nnl_constraints):
            tmp1 = self.smoother1_first_derivative(
                approx_values-approx_values[ii])
            tmp2 = self.smoother2_first_derivative(
    ...
    def define_nonlinear_constraint_hessian(self, x, ii):
        ...
        tmp1 = self.smoother1_second_derivative(
            approx_values-approx_values[ii])
...
class FSDOptProblem(SmoothDisutilitySSDOptProblem):
    ...
    def smooth_heaviside_function(self, x):
        ...
        if self.smoother_type == 2:
            if x.ndim == 1:
                x = x[:, None]
            vals1 = numba_smooth_left_heaviside_function_type2(x, self.eps)
            return vals1
    ...
    def smooth_heaviside_function_first_derivative(self, x):
        if self.smoother_type == 2:
            if x.ndim == 1:
                x = x[:, None]
            vals2 = numba_smooth_left_heaviside_function_first_derivative_type2(
                x, self.eps)
            return vals2
    ...
    def smooth_max_function_third_derivative(self, x):
        ...
        if self.smoother_type == 0:
            ...
            if x.ndim == 1:
                x = x[:, None]
            return numba_smooth_min_function_third_derivative_type2(
                x, self.eps)
        else:
            msg = "incorrect smoother_type"
            raise Exception(msg)
    ...
    def nonlinear_constraints(self, x):
        ...
        tmp1 = self.smoother1(approx_values[:, None]-approx_values[None, :])
        tmp2 = self.smoother2(self.values[:, None]-approx_values[None, :])
    ...
    def nonlinear_constraints_jacobian(self, x):
        ...
        approx_values = self.basis_matrix.dot(coef)
        tmp1 = self.smoother1_first_derivative(
            approx_values-approx_values[:, None])
        tmp2 = self.smoother2_first_derivative(
            self.values-approx_values[:, None])
        jac = self.probabilities.dot(tmp1[:, :, None]*(
            self.basis_matrix-self.basis_matrix[:, None, :]))
        jac += self.probabilities.dot(tmp2[:, :, None]*self.basis_matrix)
        return jac
    ...
    def nonlinear_constraints_hessian(self, x, v):
        ...
        approx_values = self.basis_matrix.dot(coef)
        tmp1 = self.smoother1_second_derivative(
            approx_values[:, None]-approx_values[None, :])
        tmp2 = self.smoother2_second_derivative(
            self.values[:, None]-approx_values[None, :])
        hess = np.zeros((self.nunknowns, self.nunknowns))
        for ii in range(v.shape[0]):
            # if np.any(tmp1[:, ii]!=0):
            #     tmp3 = self.basis_matrix-self.basis_matrix[ii, :]
            #     h1 = ((self.probabilities*tmp1[:, ii])[:, None]*tmp3).T.dot(tmp3)
            #     hess += v[ii]*h1
            # if np.any(tmp2[:, ii]!=0):
            #     tmp4 = self.basis_matrix*0-self.basis_matrix[ii, :]
            #     h2 = ((self.probabilities*tmp2[:, ii])[:, None]*tmp4).T.dot(tmp4)
            #     hess -= v[ii]*h2
            hessian = self.define_nonlinear_constraint_hessian(
                x, tmp1[ii], tmp2[ii], ii)
            hess += hessian*v[ii]
        return hess

    def define_nonlinear_constraint_hessian(self, x, tmp1a, tmp2a, ii):
        r"""
        d^2/dx^2 f(g(x))=g'(x)^2 f''(g(x))+g''(x)f'(g(x))
        ...
        for jj in range(self.nunknowns):
            for kk in range(jj, self.nunknowns):
                hessian[jj, kk] = self.probabilities.dot(
                    tmp1[:, 0]*(tmp3[:, jj]*tmp3[:, kk]))
                hessian[jj, kk] -= self.probabilities.dot(
                    tmp2[:, 0]*(tmp4[jj]*tmp4[kk]))
                hessian[kk, jj] = hessian[jj, kk]
        return hessian
        ...
        # hessian = np.zeros((self.nunknowns, self.nunknowns))
        # tmp2 = self.basis_matrix
        # tmp3 = (self.probabilities*tmp1[:, 0])[:, None]*tmp2
        # return tmp3.T.dot(tmp2)
