...
class TestGaussian(unittest.TestCase):
    ...
    def test_expand_scope_of_gaussian(self):
        ...
        num_new_vars = nvars_per_new_var.sum()
        true_new_matrix = np.zeros((num_new_vars,num_new_vars))
        true_new_matrix[:nvars_per_new_var[0],:nvars_per_new_var[0]]=matrix[:nvars_per_new_var[0],:nvars_per_new_var[0]]
        true_new_matrix[nvars_per_new_var[:2].sum():,:nvars_per_new_var[0]]=matrix[nvars_per_new_var[0]:,:nvars_per_new_var[0]]
        true_new_matrix[:nvars_per_new_var[0],nvars_per_new_var[:2].sum():]=matrix[:nvars_per_new_var[0],nvars_per_new_var[0]:]
        true_new_matrix[nvars_per_new_var[:2].sum():,nvars_per_new_var[:2].sum():]=matrix[nvars_per_new_var[0]:,nvars_per_new_var[0]:]
        #print(true_new_matrix)
        assert np.allclose(new_matrix,true_new_matrix)
