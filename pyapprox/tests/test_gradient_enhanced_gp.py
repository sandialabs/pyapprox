...
def gradient_enhanced_gp_example(num_vars,plot=True):
    ...
    if num_vars==2:
        ...
        const = 4
    function = lambda xx: np.cos(const*xx.sum(axis=1))
    deriv    = lambda xx: np.array(
    ...
    I = np.arange(XX_candidates.shape[0])
    XX_train_values = XX_candidates[I]
    ...
    length_scale=[1]*num_vars; noise_level=0.02; n_restarts_optimizer=3
    
    gegp_kernel  = 1*DerivGPKernel(num_XX_train_values,
    ...
    gegp.fit(XX_train_values, YY_train_values, XX_train_derivs, YY_train_derivs)
    ...
    if plot and num_vars==2:
        ...
        axs=axs.ravel()
        bounds = [-1,1,-1,1]; num_XX_test_1d=30
        ...
        plot_2d(gegp.predict,num_XX_test_1d,bounds,XX_train_values,ax=axs[2])

        deriv0 = lambda XX: deriv(XX)[:,0]
        plot_2d(deriv0,num_XX_test_1d,bounds,XX_train_values,ax=axs[3])
        #plot_2d(gp.predict,num_XX_test_1d,bounds,XX_train_values,ax=axs[1])
        plot_2d(gegp.predict_derivatives,num_XX_test_1d,bounds,XX_train_values,
                ax=axs[5])
...
class TestGradientEnhancedGP(unittest.TestCase):
    def test_fd_kernel_1d(self):
        ...
        for ii in range(num_pts):
            def f(x):
                Xf[ii]=x[0]
                return kernel_ff(Xf[:,np.newaxis],Y[:,np.newaxis],length_scale)
    ...
    def test_ff_kernel_1d(self):
        ...
        for ii in range(num_pts):
            for jj in range(ii+1,num_pts):
                x_eps = x.copy(); x_eps[ii]+=eps
                y_eps = y.copy(); y_eps[jj]+=eps
                k_eps = kernel_ff(
        ...
        K_dd =  kernel_dd(Xf[:,np.newaxis],Xf[:,np.newaxis],length_scale,0,0)
        #print('K_dd',K_dd)
