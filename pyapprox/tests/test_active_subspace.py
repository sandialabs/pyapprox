...
class TestActiveSubspace(unittest.TestCase):
    ...
    def test_evaluate_active_subspace_density_1d(self):
        ...
        uniform_density_fn = lambda x: np.ones(x.shape[1])*0.25
        evaluate_active_subspace_density_1d_example(
            uniform_density_fn,1e-3,test=True)

    def test_coeffs_of_active_subspace_polynomial(self):
        num_vars=3; num_active_vars = 2; degree=4;
    ...
    def test_moments_of_active_subspace(self):
        num_vars=3; num_active_vars = 2; degree=2;
        A = np.random.normal(0,1,(num_vars,num_vars))
        Q, R = np.linalg.qr( A )
        W1 = Q[:,:num_active_vars]
        ...
        true_moments = [1,0,0,
                        np.sum(W1[:,1]**2)*1./3.,# notice how if W1 has colums with unit norm np.sum(W1[:,1]**2) will always be one.
                        1./3.*(W1[0,0]*W1[0,1]+W1[1,0]*W1[1,1]+W1[2,0]*W1[2,1]),
                        np.sum(W1[:,0]**2)*1./3.]
        ...
        true_moments = np.array([1,0,0,np.sum(W1[:,1]**2)*1./3.,
                        ...
                        (3*d**4+3*e**4+10*e**2*f**2+3*f**4+10*d**2*(e**2+f**2))/15.,
                        dummy,dummy,dummy,
                        (3*a**4+3*b**4+10*b**2*c**2+3*c**4+10*a**2*(b**2+c**2))/15.])
        moments = moments[sorted_idx]
        I = np.where(true_moments!=np.Inf)[0]#ignore dummy values until I compute them analytically
