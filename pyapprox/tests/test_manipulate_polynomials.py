...
class TestManipulatePolynomials(unittest.TestCase):
    ...
    def test_substitute_polynomial_for_variables_in_single_basis_term(self):
        ...
        and
          ...
          y3 = y1**2*x4**3*y2 (test2)
             = 2*x4**3*x1**3*x2**2*x3+2*x4**3*x1**3*x2**2+4*x4**3*x1**3*x2*x3+4*x4**3*x1**3*x2+2*x4**3*x1**3*x3+2*x4**3*x1**3+4*x4**3*x1**2*x2**2*x3+6*x4**3*x1**2*x2**2+8*x4**3*x1**2*x2*x3+12*x4**3*x1**2*x2+4*x4**3*x1**2*x3+6*x4**3*x1**2+2*x4**3*x1*x2**2*x3+6*x4**3*x1*x2**2+4*x4**3*x1*x2*x3+12*x4**3*x1*x2+2*x4**3*x1*x3+6*x4**3*x1+2*x4**3*x2**2+4*x4**3*x2+2*x4**3
(21 terms)
        21 terms
        ...
        new_indices, new_coeffs  = \
            substitute_polynomial_for_variables_in_single_basis_term(
                ...
                global_var_idx)
        true_coeffs = np.array([[2, 4, 2, 2, 2, 2, 4, 2, 2, 2]]).T
        assert new_coeffs.shape[0] == new_indices.shape[1]
        assert new_indices.shape[1] == 10
        assert np.allclose(true_coeffs, new_coeffs)
        ...
        true_coeffs = np.array(
            [[2, 6, 2, 6, 2, 2, 2, 6, 2, 6, 2, 2, 4, 4, 12, 4, 12,
              8, 4, 4, 4]]).T
        assert np.allclose(true_coeffs, new_coeffs)
    ...
    def test_substitute_polynomials_for_variables_in_another_polynomial(self):
        ...
        true_coef = np.linalg.lstsq(basis_mat, values, rcond=None)[0]
        ...
        assert np.allclose(
            validation_values, validation_basis_mat.dot(true_coef), rtol=1e-12)
        ...
        I = np.where(np.absolute(true_coef)>1e-12)[0]
        true_indices = true_indices[:, I]
        true_coef = true_coef[I]
        sorted_idx = argsort_indices_leixographically(true_indices)
        true_indices = true_indices[:, sorted_idx]
        true_coef = true_coef[sorted_idx]
        sorted_idx = argsort_indices_leixographically(new_indices)
        new_indices = new_indices[:, sorted_idx]
        new_coef = new_coef[sorted_idx]
        assert np.allclose(new_indices, true_indices)
        # print(new_coef[:, 0])
        # print(true_coef)
        assert np.allclose(new_coef[:, 0], true_coef)
