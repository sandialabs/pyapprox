...
class TestClaytonCopula(Generic[Array], unittest.TestCase):
    ...
    def setUp(self) -> None:
        self._bkd = self.bkd()
        self._theta = 2.0
        self._copula = ClaytonCopula(self._theta, self._bkd)
    ...
    def test_sample_kendall_tau(self) -> None:
        ...
        expected_tau = self._theta / (self._theta + 2.0)
    ...
    def test_kendall_tau_value(self) -> None:
        ...
        expected = self._theta / (self._theta + 2.0)
    ...
    def test_strong_dependence(self) -> None:
        ...
        copula = ClaytonCopula(10.0, self._bkd)
        np.random.seed(42)
...
class TestClaytonCopulaNumpy(TestClaytonCopula[NDArray[Any]]):
    def bkd(self) -> NumpyBkd:
        return NumpyBkd()


class TestClaytonCopulaTorch(TestClaytonCopula[torch.Tensor]):
