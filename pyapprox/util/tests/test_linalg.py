...
from pyapprox.util.linalg import (
    ...
    add_rows_to_pivoted_lu_factorization,
...
class TestLinalg(unittest.TestCase):
    ...
    def test_split_lu_factorization_matrix(self):
        ...
        assert np.allclose(
            L_factor.dot(U_factor), pivot_rows(pivots, A, False)
        )
    ...
    def test_unprecondition_LU_factor(self):
        ...
        unprecond_LU_factor, unprecond_pivots = (
            truncated_pivoted_lu_factorization(
                A, num_pivots, truncate_L_factor=False, num_initial_rows=pivots
            )
        )
    ...
    def test_cholesky_decomposition(self):
        nrows = 4
        A = np.random.normal(0.0, 1.0, (nrows, nrows))
        A = A.T.dot(A)
    ...
    def test_pivoted_cholesky_decomposition(self):
        ...
        nrows, npivots = 4, 2
        A = np.random.normal(0.0, 1.0, (npivots, nrows))
        A = A.T.dot(A)
        L, pivots, error, flag = pivoted_cholesky_decomposition(A, npivots)
        ...
        nrows, npivots = 4, 2
        A = np.random.normal(0.0, 1.0, (npivots + 1, nrows))
        A = A.T.dot(A)
        ...
        A = np.array([[4, 12, -16], [12, 37, -43], [-16, -43, 98.0]])
    ...
    def test_cholesky_decomposition_minimizing_trace_norm(self):
        ...
        for ii in range(n):
            ...
            assert np.allclose(
                A[np.ix_(pivots[: ii + 1], pivots[: ii + 1])],
                A_ii[: ii + 1, : ii + 1],
            )
            assert np.allclose(
                (A[np.ix_(pivots, pivots)] - A_ii)[ii + 1 :, ii + 1 :], S
            )
            assert np.allclose(np.trace(A), np.trace(A_ii) + np.trace(S))
            assert np.allclose(np.trace(S), np.trace(A) - np.trace(A_ii))
    ...
    def test_num_entries_triangular_matrix(self):
        ...
        A = np.ones([M, N])
        L = np.tril(A)
        nentries = num_entries_rectangular_triangular_matrix(M, N, upper=False)
        assert nentries == np.count_nonzero(L)

        A = np.ones([M, N])
        U = np.triu(A)
        nentries = num_entries_rectangular_triangular_matrix(M, N, upper=True)
    ...
    def test_qr_solve(self):
        nrows = 3
        rhs = np.random.normal(0.0, 1.0, (nrows, 1))
        A = np.random.normal(0.0, 1.0, (nrows, nrows))
        A = A.T.dot(A)
    ...
    def test_inverse_of_cholesky_factor(self):
        nrows = 3
        A = np.random.normal(0.0, 1.0, (nrows, nrows))
        A = A.T.dot(A)
        L = cholesky_decomposition(A)
        A_inv = cholesky_inverse(L)
        L_inv = inverse_of_cholesky_factor(L)
        assert np.allclose(L_inv, np.linalg.inv(L))
        assert np.allclose(L_inv.T @ L_inv, np.linalg.inv(A))
        assert np.allclose(A_inv, np.linalg.inv(A))
        assert np.allclose(
            log_determinant_from_cholesky_factor(L), np.log(np.linalg.det(A))
        )

    def test_trace_of_mat_mat_product(self):
        nrows, ncols = 3, 4
        Amat = np.random.normal(0.0, 1.0, (nrows, ncols))
        Bmat = Amat.T
