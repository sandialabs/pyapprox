...
class TestHyperParameter:
    ...
    def test_hyperparameter(self):
        ...
        hyp_1 = HyperParameter("P1", 2, -0.5, [-1, 6, -2, 3], transform_1)
        hyp_1.set_active_indices(
            bkd.asarray(
                [
                    0,
                ],
        ...
        assert np.allclose(
            hyp_list_0.get_active_opt_bounds(),
            np.vstack(
                (
                    np.log(np.array([[0.01, 2], [0.01, 2], [0.01, 2]])),
                    np.array([[-1, 6]]),
        ...
        assert np.allclose(
            hyp_list_2.get_values(),
            np.hstack((np.full(3, 1), np.full(2, -0.5), np.full(1, 0.25))),
        )
        assert np.allclose(
            hyp_list_2.get_active_opt_bounds(),
            np.vstack(
                (
                    np.log(np.array([[0.01, 2], [0.01, 2], [0.01, 2]])),
                    np.array([[-1, 6]]),
    ...
    def test_cholesky_hyperparameter(self):
        bkd = self.get_backend()
        nrows = 3
        A = bkd.asarray(np.random.normal(0, 1, (nrows, nrows)))
        chol = bkd.cholesky(A.T @ A)
        mask = bkd.tril(bkd.ones((nrows, nrows), dtype=bool))
        chol_flat = bkd.tril(chol)[mask].flatten()
        hyp = CholeskyHyperParameter("cholesky", nrows, chol_flat, backend=bkd)
        recovered_chol = hyp.get_cholesky_factor()
        assert bkd.allclose(recovered_chol, chol)
