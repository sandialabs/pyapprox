...
class TestUtilities(unittest.TestCase):
    ...
    def test_cartesian_product(self):
        ...
        sets = np.array(
            [[0, 3], [1, 3], [2, 3], [0, 4], [1, 4], [2, 4]], np.int64
        )
    ...
    def test_outer_product(self):
        ...
        test_vals = np.array([0.0, 3.0, 6.0, 0.0, 4.0, 8.0])
    ...
    def test_leave_many_out_lsq_cross_validation(self):
        ...
        for kk in range(len(fold_sample_indices)):
            ...
            gram_mat_kk = (
                basis_mat_kk.T.dot(basis_mat_kk)
                + np.eye(basis_mat_kk.shape[1]) * alpha
            )
            values_kk = basis_mat_kk.T.dot(values[K, :])
            coef_kk = np.linalg.lstsq(gram_mat_kk, values_kk, rcond=None)[0]
            true_cv_errors[kk] = (
                basis_mat[fold_sample_indices[kk], :].dot(coef_kk)
                - values[fold_sample_indices[kk]]
            )
        # print(cv_errors, true_cv_errors)
        assert np.allclose(cv_errors, true_cv_errors)
        true_cv_score = np.sqrt(
            (true_cv_errors**2).sum(axis=(0, 1)) / nsamples
        )
