...
class TestGaussian:
    ...
    def _check_multivariate_gaussian(self, variable):
        mean = variable.mean()
        ...
        assert np.allclose(variable.covariance_diagonal(), np.diag(cov))
    ...
    def test_condition_gaussian(self):
        ...
        true_cond_mean = mean[remain_indices] + sigma[remain_indices] / sigma[
            fixed_indices
        ] * correlation * (values - mean[fixed_indices])
    ...
    def test_joint_density_from_linear_conditional_relationship(self):
        ...
        temp = np.random.normal(0.0, 1.0, (nvars1, nvars1))
        cov1 = temp.T.dot(temp)
        temp = np.random.normal(0.0, 1.0, (nvars2, nvars2))
    ...
    def generate_gaussian_in_canonical_random(self, nvars):
        ...
        mean = np.random.normal(0.0, 1.0, nvars)
    ...
    def test_expand_scope_of_gaussian(self):
        ...
        true_new_matrix[
            nvars_per_new_var[:2].sum() :, : nvars_per_new_var[0]
        ] = matrix[nvars_per_new_var[0] :, : nvars_per_new_var[0]]
        true_new_matrix[
            : nvars_per_new_var[0], nvars_per_new_var[:2].sum() :
        ] = matrix[: nvars_per_new_var[0], nvars_per_new_var[0] :]
        true_new_matrix[
            nvars_per_new_var[:2].sum() :, nvars_per_new_var[:2].sum() :
        ] = matrix[nvars_per_new_var[0] :, nvars_per_new_var[0] :]
        # print(true_new_matrix)
        assert np.allclose(new_matrix, true_new_matrix)
