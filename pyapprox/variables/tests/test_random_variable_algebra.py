...
from pyapprox.variables.algebra import (
    ...
    scalar_multiple_of_random_variable,
    power_of_random_variable_pdf,
    get_global_maxima_and_minima_of_monomial_expansion,
    get_pdf_from_monomial_expansion,
    get_cdf_from_monomial_expansion,
...
class TestRandomVariableAlgebra(unittest.TestCase):
    ...
    def test_get_inverse_derivatives_cubic_polynomial(self):
        ...
        inverse_vals, inverse_derivs, defined_indices = (
            get_inverse_derivatives(poly, [lb, ub], zz)
        )
    ...
    def test_variable_transformation_uniform_cubic_polynomial(self):
        ...
        poly = np.poly1d(coef[::-1])
        poly_min, poly_max = (
            get_global_maxima_and_minima_of_monomial_expansion(poly, lb, ub)
        )
    ...
    def test_variable_transformation_uniform_cosine_taylor_series_expansion(
        ...
        assert np.allclose(z_pdf_vals, fd_z_pdf_vals, atol=6e-5), (
            z_pdf_vals - fd_z_pdf_vals
        )
    ...
    def test_variable_transformation_uniform_postive_poly(self):
        ...
        x_cdf = stats.uniform(lb, ub - lb).cdf
        zz = np.linspace(
            min(poly([lb, ub]).min(), poly(critical_points).min()) * 1.001,
            max(poly([lb, ub]).max(), poly(critical_points).max()) * 0.999,
            101,
        )
        z_cdf_vals = get_cdf_from_monomial_expansion(coef, lb, ub, x_cdf, zz)
    ...
    def test_sum_of_independent_uniform_and_gaussian_variables(self):
        ...
        conv_pdf = sum_of_independent_random_variables_pdf(pdf2, [[x, w]], zz)

        def conv_pdf_exact(zz):
            return (
                1
                / (2 * (b - a))
                * (erf((b - zz) / np.sqrt(2)) - erf((a - zz) / np.sqrt(2)))
            )
    ...
    def test_sum_of_independent_gaussian_variables(self):
        ...
        pdfs = [
            stats.norm(loc=means[ii], scale=np.sqrt(variances[ii])).pdf
        ...
        x, w = scipy_gauss_hermite_pts_wts_1D(100)
        quad_rules = [
            [x * np.sqrt(variances[ii]) + means[ii], w]
        ...
        true_pdf = stats.norm(
            loc=means.sum(), scale=np.sqrt(variances.sum())
        ).pdf
    ...
    def test_product_of_independent_gaussian_variables(self):
        ...
        def exact_product_two_zero_mean_independent_gaussians(var1, var2, zz):
            ...
            vals = modified_bessel_2nd_kind(0, np.absolute(zz) / tmp) / (
                np.pi * tmp
            )
            return vals
    ...
    def test_weighted_sum_dependent_gaussian_variables(self):
        ...
        assert np.allclose(
            sum_var,
            [
                (weights[:, 0] ** 2 * np.diag(covariance)).sum(),
        ...
        nsamples = int(1e6)
        samples = (
            chol_factor.dot(np.random.normal(0, 1, (nvars, nsamples))) + mean
        )
