...
class TestProbabilitySampling(unittest.TestCase):
    def setUp(self):
        ...
        self.continuous_mean = np.array(
            [
                ...
                0.5,
                0.0,
                stats.beta.mean(a=2, b=1, loc=-2, scale=3),
            ]
        )
    ...
    def test_independent_continuous_samples(self):
        ...
        variable = IndependentMarginalsVariable(self.continuous_variables)

        var_trans = AffineTransform(variable)

        nsamples = int(1e6)
        samples = var_trans.variable().rvs(nsamples)
    ...
    def test_independent_discrete_samples(self):
        ...
        variable = IndependentMarginalsVariable(self.discrete_variables)
        var_trans = AffineTransform(variable)
    ...
    def test_independent_mixed_continuous_discrete_samples(self):
        ...
        true_mean = np.concatenate([self.continuous_mean, self.discrete_mean])[
            II
        ]
