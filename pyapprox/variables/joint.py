...
class MultivariateGaussian(JointVariable):
    ...
    def _set_covariance(self, cov: Array):
        ...
        self._cov = cov
        self._cov_sqrt = self._bkd.cholesky(cov)
    ...
    def log_pdf_jacobian(self, samples: Array) -> Array:
        return -(
            self._cov_sqrt_inv.T
            @ (self._cov_sqrt_inv @ (samples - self._mean))
        ).T
...
class IndependentMultivariateGaussian(MultivariateGaussian):
    def _set_covariance(self, cov_diag: Array):
        if cov_diag.shape != (self.nvars(),):
            raise ValueError("cov_diag has the wrong shape")
