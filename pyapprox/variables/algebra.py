...
def scalar_multiple_of_random_variable(pdf_func, coefficient, xx):
    ...
    if abs(coefficient) > 0:
        return pdf_func(xx / coefficient) * abs(1.0 / coefficient)
    else:
...
def power_of_random_variable_pdf(pdf_func, power, xx):
    ...
    vals[II] = pdf_func(xx[II] ** root) * np.absolute(
        root * (xx[II] ** (root - 1))
    )
...
def invert_monotone_function(poly, bounds, zz, method="newton", tol=1e-12):
    ...
    for ii in range(zz.shape[0]):
        ...
        flag1 = (
            np.isfinite(lb)
            and np.isfinite(ub)
            and np.sign(func(lb)) == np.sign(func(ub))
        )
        flag2 = (
            np.isfinite(lb)
            and not np.isfinite(ub)
            and deriv_poly(lb) < 0
            and func(lb) < 0
        )
        flag3 = (
...
def get_inverse_derivatives(
    poly, bounds, zz, fd_eps=np.sqrt(np.finfo(float).eps)
):
    ...
    defined_inverse_derivs = (
        defined_inverse_vals_perturbed - defined_inverse_vals
    ) / fd_eps
...
def get_pdf_from_monomial_expansion(coef, lb, ub, x_pdf, zz):
    ...
    if (
        len(intervals) == 0
        or not np.isfinite(intervals[0])
        or abs(intervals[0] - lb) > 1e-15
    ):
    ...
    for jj in range(intervals.shape[0] - 1):
        inverse_vals, inverse_derivs, defined_indices = (
            get_inverse_derivatives(poly, intervals[jj : jj + 2], zz)
        )
    ...
    for ii in range(zz.shape[0]):
        for jj in range(intervals.shape[0] - 1):
            ...
            if np.isfinite(inverse_val):
                ...
                if decreasing:
                    zz_cdf_vals[ii] += x_cdf(intervals[jj + 1]) - x_cdf(
                        inverse_val[0]
                    )
                else:
                    zz_cdf_vals[ii] += x_cdf(inverse_val[0]) - x_cdf(
                        intervals[jj]
                    )
