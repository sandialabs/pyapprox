...
This module provides a Halton sequence sampler using scipy.stats.qmc,
...
@runtime_checkable
class DistributionWithInvCDF(Protocol, Generic[Array]):
    """Protocol for distributions that support inverse CDF transformation."""
    ...
    def invcdf(self, probs: Array) -> Array:
        """Transform uniform samples via inverse CDF."""
        ...
...
class HaltonSampler(Generic[Array]):
    ...
    Generates low-discrepancy samples using the Halton sequence via scipy.
    ...
    >>> sampler = HaltonSampler(3, bkd, scramble=False)
    ...
    >>> sampler = HaltonSampler(3, bkd, distribution=dist)
    ...
    def _reset_engine(self) -> None:
        """Create or reset the Halton engine."""
        from scipy.stats import qmc

        self._engine = qmc.Halton(
