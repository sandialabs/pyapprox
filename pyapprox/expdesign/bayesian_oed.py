...
def gaussian_loglike_fun_broadcast(
    ...
    """
    if type(noise_std) == np.ndarray and noise_std.shape[0] != obs.shape[-1]:
...
def compute_weighted_sqeuclidian_distance(
    obs, pred_obs, noise_std, active_indices
):
...
def gaussian_loglike_fun_economial_2D(
    ...
    if active_indices is None:
        llike = (
            0.5 * np.sum(np.log(1 / (2 * np.pi * noise_std[:, 0] ** 2)))
            - sq_dists[0, :]
        )
    else:
        llike = (
...
def _evidences(inner_log_likelihood_vals, in_weights):
    ...
    for mm in range(MM):
        ...
        for nn in range(NN):
            evidences[mm, 0] += (
                math.exp(inner_log_likelihood_vals[mm, nn])
                * in_weights[mm, nn]
            )
    return evidences
...
def _loglike_fun_from_noiseless_obs(
    noiseless_obs, pred_obs, noise_realizations, noise_std, active_indices
):
    nactive_indices = active_indices.shape[0]
...
def _compute_evidences_repeated_in_samples(
    ...
    for ii in range(nout_samples):
        ...
        for jj in range(nin_samples):
            ...
            for kk in range(nactive_indices):
                loglike_val += (
                    const1[active_indices[kk]]
                    * (out_obs[ii, kk] - in_pred_obs[jj, active_indices[kk]])
                    ** 2
                )
...
def _compute_evidences(
    ...
    outer_log_likelihood_vals = _loglike_fun_from_noiseless_obs(
        out_pred_obs, out_pred_obs, noise_samples, noise_std, active_indices
    )
...
def _precompute_out_quadrature_rule(
    ...
    """
    out_samples, out_weights = generate_prior_noise_samples(nout_samples)
    out_prior_samples = out_samples[:nvars]
    out_noise_samples = out_samples[nvars:]
    out_prior_quad_data = (out_prior_samples, out_weights)
...
def _precompute_expected_kl_utility_data(out_quad_data, in_quad_data, obs_fun):
    ...
    print(
        "Predicted observation bounds", out_pred_obs.max(), out_pred_obs.min()
    )
...
def _evidences_and_weights(inner_log_likelihood_vals, in_weights):
    ...
    evidences = np.einsum("ij,ij->i", inner_likelihood_vals, in_weights)[
        :, None
    ]
...
def _define_design_data(
    ...
    noise_std = np.hstack(
        [
            ...
            for idx in range(ndesign_candidates)
        ]
    )[:, None]
...
def oed_data_expectation(deviations, weights):
    ...
    expected_obs_deviations : np.ndarray (nqois, 1)
        The deviation vals
    """
    expected_obs_deviations = np.einsum("ij,i->j", deviations, weights[:, 0])[
        :, None
    ]
...
class AbstractBayesianOED(ABC):
    ...
    def __init__(
        ...
        noise_variable_marginals = [
            stats.norm(0, self.noise_std[0, 0])
        ...
        (
            ...
            self.in_quad_data,
            self.econ,
        ) = self._get_quad_rules(out_quad_opts, in_quad_opts)
    ...
    def _get_quad_rules(self, out_quad_opts, in_quad_opts):
        out_quad_data = integrate(
            out_quad_opts["method"],
            self.joint_prior_noise_variable,
            *out_quad_opts.get("args", []),
            **out_quad_opts.get("kwargs", []),
        )
        out_prior_samples = out_quad_data[0][: self.prior_variable.num_vars()]
        out_noise_samples = out_quad_data[0][
            self.prior_variable.num_vars() :
        ].T
        out_quad_data = (out_prior_samples, out_quad_data[1])
        in_quad_data = integrate(
            in_quad_opts["method"],
            self.prior_variable,
            *in_quad_opts.get("args", []),
            **in_quad_opts.get("kwargs", []),
        )
        econ = True
    ...
    def update_design(self, return_all=False, nnew=1, with_replacement=True):
        ...
        if (
            self.collected_design_indices.shape[0] + nnew
            > self.max_ncollected_obs
        ):
        ...
        self.collected_design_indices = np.hstack(
            (self.collected_design_indices, selected_indices)
        ).astype(int)
    ...
    def compute_expected_utility(
        self, collected_design_indices, new_design_indices, return_all=False
    ):
        raise NotImplementedError()
    ...
    def _compute_utilities_parallel_shared(
        ...
        args = [
            (
                indices[splits[ii] : splits[ii + 1]],
                collected_design_indices,
                return_all,
                self.ndesign_candidates,
                self.ndata_per_candidate,
            )
    ...
    def compute_utilities(
        self, ncandidates, collected_design_indices, new_indices, return_all
    ):
        if self.nprocs == 1:
        ...
        return self._compute_utilities_parallel_shared(
            kl_worker_fun,
            init_kl_worker,
            _get_kl_compute_utilities_initargs,
            new_indices,
            collected_design_indices,
            return_all,
        )
    ...
    def select_design(
        ...
        if new_indices is None:
            if with_replacement:
                ...
                print(
                    "No. total {0} pt designs: {1}".format(
                ...
                new_indices = np.asarray(
                    list(
                        itertools.combinations(
                            np.delete(
                                np.arange(self.ndesign_candidates),
                                collected_design_indices,
                            ),
                            nnew,
                        )
...
class OEDSharedData:
    ...
    def set_funs(self, deviation_fun, pred_risk_fun, data_risk_fun):
...
@njit(cache=True)
def _posterior_push_fwd_entropic_deviation(qoi_vals, nin_samples, weights):
    qoi_mean = np.zeros(qoi_vals.shape[1])
    deviations = np.zeros(qoi_vals.shape[1])
    for jj in range(nin_samples):
        qoi_mean += qoi_vals[jj, :] * weights[jj]
...
@njit(cache=True)
def _posterior_push_fwd_cvar_deviation(qoi_vals, nin_samples, weights, beta):
    ...
    risks = conditional_value_at_risk_vectorized(
        qoi_vals.copy().T, beta, weights_expanded, samples_sorted=False
    )
...
def oed_variance_deviation(samples, weights):
    ...
    try:
        ...
        pass
    means = np.einsum("ijk,ij->ik", samples, weights)
    variances = np.einsum("ijk,ij->ik", samples**2, weights) - means**2
    return variances
...
def oed_conditional_value_at_risk_deviation(
    samples, weights, quantile=None, samples_sorted=True
):
    """
...
class BayesianBatchDeviationOED(AbstractBayesianOED):
    ...
    def _populate(self):
        ...
        """
        (self.out_pred_obs, self.in_pred_obs, self.in_pred_qois) = (
            _precompute_expected_deviation_data(
                self.out_quad_data,
                self.in_quad_data,
                self.obs_fun,
                self.qoi_fun,
    ...
    def compute_utilities(
        ...
        return self._compute_utilities_parallel_shared(
            deviation_worker_fun,
            init_deviation_worker,
...
class BayesianSequentialOED(AbstractBayesianOED):
    ...
    def compute_importance_weights(self):
        ...
        """
        self.outer_importance_weights = np.exp(
            self._loglike_fun(
                self.collected_obs,
                self.out_pred_obs[:, self.collected_design_indices],
                self.noise_std[self.collected_design_indices],
            )
    ...
    def update_observations(self, new_obs):
        ...
        if self.collected_obs is None:
            ...
            self.collected_obs = np.hstack((self.collected_obs, new_obs))
        self._compute_evidence()
        self.compute_importance_weights()
        self.out_weights_up = self.out_weights * self.outer_importance_weights
        self.in_weights_up = self.in_weights * self.inner_importance_weights
...
def get_oed_inner_quadrature_rule(
    nin_samples, prior_variable, quad_method="gauss"
):
    """
    Parameters
    ----------
    ...
    if quad_method == "gauss":
        ...
        univariate_quad_rules = get_univariate_quadrature_rules_from_variable(
            prior_variable, [nin_samples_1d] * nrandom_vars
        )[0]
        x_quad, w_quad = get_tensor_product_quadrature_rule(
            [nin_samples_1d] * nrandom_vars,
            nrandom_vars,
    ...
    new_ranges = prior_variable.get_statistics(
        "interval", confidence=alpha
    ).flatten()
    x_quad, w_quad = get_tensor_product_piecewise_polynomial_quadrature_rule(
...
def get_posterior_weights_at_in_samples(oed, nn, out_idx):
    ...
    active_indices = np.hstack(
        [
            idx * oed.ndata_per_candidate + np.arange(oed.ndata_per_candidate)
            for idx in oed.collected_design_indices[:nn]
        ]
    )
    outer_log_likelihood_vals, evidences = _compute_evidences(
        ...
        oed.in_pred_obs,
        oed.in_weights,
        oed.out_weights[out_idx : out_idx + 1],
        active_indices,
...
def get_posterior_2d_interpolant_from_oed_data(
    ...
    if quad_method == "gauss":
        ...
        for dd in range(nvars):
            abscissa_1d.append(np.unique(oed.in_samples[dd, :nin_samples]))
        fun = partial(
            tensor_product_barycentric_interpolation, abscissa_1d, vals
        )
        return fun
    ...
    def fun(x):
        return griddata(x_quad.T, vals, x.T, method="linear")

    return fun


def plot_2d_posterior_from_oed_data(
    oed, prior_variable, nn, out_idx, method, ax=None, oed_results=None
):
    ...
    p = ax.contourf(X, Y, Z, levels=np.linspace(Z.min(), Z.max(), 21))
...
def tensor_product_barycentric_interpolation(abscissa_1d, values, samples):
    ...
    poly_vals = multivariate_barycentric_lagrange_interpolation(
        samples, abscissa_1d, barycentric_weights_1d, values, np.arange(nvars)
    )
    return poly_vals
...
def get_data_risk_fun(name, opts={}):
    ...
    deviation_fun : callable
        ...
        `deviation_fun(in_pred_qois, weights) ->
         ...
         weights : np.ndarray (nout_samples, nin_samples)
    """
    risk_funs = {"mean": oed_data_expectation, "cvar": oed_data_cvar}
...
def gaussian_noise_fun(noise_std, values, active_indices=None):
    ...
    return np.random.normal(0, noise_std[active_indices], shape)
...
def get_bayesian_oed_optimizer(
    ...
    **kwargs,
    ...
    oed_types = {
        "open_loop_kl_params": BayesianBatchKLOED,
    ...
    if (
        type(noise_std) == np.ndarray
        and noise_std.shape[0] != ndesign_candidates
    ):
