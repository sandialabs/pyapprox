...
class TestScipyTrustConstrOptimizer:
    ...
    def test_optimizer_with_evtushenko_objective_and_constraints(self, bkd) -> None:
        ...
        # called.
        assert result.get_raw_result().nfev > 0
        assert result.get_raw_result().njev > 0
        assert result.get_raw_result().nhev > 0

        # Check that function, jacobian and whvp of constraints are
        # called.
        assert any(bkd.asarray(result.get_raw_result().constr_nfev) > 0)
        assert any(bkd.asarray(result.get_raw_result().constr_njev) > 0)
        assert any(bkd.asarray(result.get_raw_result().constr_nhev) > 0)
