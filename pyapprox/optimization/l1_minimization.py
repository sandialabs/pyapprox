...
def nonlinear_basis_pursuit(
    func, func_jac, func_hess, init_guess, options, eps=0, return_full=False
):
...
def basis_pursuit_denoising(
    func, func_jac, func_hess, init_guess, eps, options
):
...
def s_sparse_projection(signal, sparsity):
    ...
    projected_signal[np.argsort(np.absolute(signal))[:-sparsity]] = 0.0
    ...
    for ii in range(maxiter):
        ...
        if verbosity > 0:
            if ii % 10 == 0:
                print(
                    (
                        "iteration %d the residual norm is %1.2e"
    ...
    while True:
        ...
        if verbosity > 0:
            if active_indices.shape[0] > 0:
                print(
                    (
                        repr(active_indices.shape[0]).center(8),
                        repr(active_indices[-1]).center(5),
