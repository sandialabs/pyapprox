...
from pyapprox.optimization.l1_minimization import (
    basis_pursuit,
    kouri_smooth_l1_norm,
    nonlinear_basis_pursuit,
    kouri_smooth_l1_norm_gradient,
    kouri_smooth_l1_norm_hessian,
...
# TODO these tests pass but need to start using backend api and new
# class structures
...
class TestL1Minimization(unittest.TestCase):
    def test_basis_pursuit(self):
        ...
        basis_matrix = samples.T ** np.arange(degree + 1)[None, :]
    ...
    def test_least_squares(self):
        ...
        def objective(x):
            ...
            obj = 0.5 * residual.T.dot(residual)
            return obj
        ...
        def hessian(x):
            return (basis_matrix.T.dot(basis_matrix))[None, ...]

        objective = ModelFromSingleSampleCallable(
            1,
            true_coef.shape[0],
            objective,
            jacobian=jacobian,
        ...
        # init_guess = lstsq_coef + np.random.normal(
        #     0, 1e-3, (true_coef.shape[0], 1)
        # )

        errors = objective.check_apply_jacobian(init_guess, disp=True)
        assert errors.min() < 2e-7
        errors = objective.check_apply_hessian(init_guess, disp=True)
        assert errors.min() < 2e-7
        ...
        optimizer = ScipyConstrainedOptimizer(objective, opts=options)
        res = optimizer.minimize(init_guess)
        # print(res.x, true_coef)
        assert np.allclose(res.x, true_coef, atol=5e-4)
    ...
    def test_nonlinear_basis_pursuit(self):
        ...
        def fun(x):
            return (
                basis_matrix.dot(x[:-1, 0])
                * np.exp(samples[0, :] * x[-1, 0])[None, :]
            )
        ...
        def jac(x):
            ...
            return np.hstack(
                [
                    basis_matrix * np.exp(samples[0, :] * x[-1, 0])[:, None],
                    (samples[0, :] * val).T,
                ]
            )

        model = ModelFromSingleSampleCallable(
            degree, basis_matrix.shape[1] + 1, fun, jac
        )

        true_coef = np.zeros((basis_matrix.shape[1] + 1, 1))
        true_coef[
            np.random.permutation(true_coef.shape[0] - 1)[: sparsity - 1]
        ] = 1.0
        true_coef[-1] = 1
        vals = model(true_coef)[0]

        init_guess = true_coef + np.random.normal(
            0, 1, (true_coef.shape[0], 1)
        )
        errors = model.check_apply_jacobian(init_guess, disp=True)
        assert errors.min() / errors.max() < 1e-6

        def func(x):
            x = x[:, None]
            return model(x)[0, :] - vals, model.jacobian(x)
        ...
        init_guess = true_coef[:, 0] + np.random.normal(
            0, 1, true_coef.shape[0]
        )
        l1_coef = nonlinear_basis_pursuit(func, jac, hess, init_guess, options)
        # print(np.linalg.norm(true_coef - l1_coef))
        assert np.allclose(l1_coef, true_coef[:, 0], atol=2e-6)
    ...
    def test_smooth_l1_norm_gradients(self):
        ...
        model = ModelFromSingleSampleCallable(
            1,
            t.shape[0],
            lambda x: np.atleast_2d(kouri_smooth_l1_norm(t, r, x[:, 0])),
            jacobian=lambda x: kouri_smooth_l1_norm_gradient(t, r, x[:, 0])[
                None, :
            ],
            hessian=lambda x: kouri_smooth_l1_norm_hessian(t, r, x[:, 0])[
                None, ...
            ],
        )
        errors = model.check_apply_jacobian(init_guess, disp=False)
        assert errors.min() < 3e-7
        errors = model.check_apply_hessian(init_guess, disp=False)
        assert errors.min() < 3e-7
    ...
    def test_lasso(self):
        ...
        for i, lamda in enumerate(lamdas):
            ...
            print(
                lamda,
                np.absolute(coef).sum(),
                func(coef, False),
                np.linalg.norm(true_coef - coef),
            )
    ...
    def test_iterative_hard_thresholding_gaussian_matrix(self):
        ...
        Amatrix = np.random.normal(
            0.0, 1.0, (num_samples, num_terms)
        ) / np.sqrt(num_samples)
        true_sol = np.zeros((num_terms))
        II = np.random.permutation(num_terms)[:sparsity]
        true_sol[II] = np.random.normal(0.0, 1.0, (sparsity))
        ...
        def apply_approx_adjoint_jacobian(x, y):
            return -np.dot(Amatrix.T, y)
    ...
    def test_omp_gaussian_matrix(self):
        ...
        def least_squares_regression(indices, initial_guess):
            return np.linalg.lstsq(Amatrix[:, indices], obs, rcond=None)[0]
