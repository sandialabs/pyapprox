...
class TestFirstOrderStochasticDominance(unittest.TestCase):
    ...
    def test_objective_derivatives(self):
        ...
        objective.set_opt_problem(problem)
        errors = objective.check_apply_jacobian(x0)
        assert errors.min() / errors.max() < 1e-6
        errors = objective.check_apply_hessian(x0)
        # fd hessian  error should decay linearly (assuming first order fd)
        # because hessian is constant (indendent of x)
        assert errors[0] < 1e-14 and errors[10] > 1e-9
        ...
        errors = constraint.check_apply_hessian(x0, weights=lmult, disp=True)
        assert errors.min() / errors.max() < 2e-7
