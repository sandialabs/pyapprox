...
# TODO implement classes like the skeletone below
# class SmoothMaxFunction(Model):
#     def __init__(self, smoother_type: str, backend: BackendMixin = NumpyMixin):
#         self._type = smoother_type
...
#         return smooth_max_function(self._type, self._eps, samples[:, 0])

#     def _jacobian(self, samples: Array):
#         return smooth_max_function_first_derivative(
#             self._type, self._eps, samples[:, 0]
#         )[None, :]


# class SmoothConditionalValueAtRisk(SmoothMaxFunction):
#     pass
...
def smooth_conditional_value_at_risk_split(
    ...
    if weights is None:
        return t + smooth_max_function(smoother_type, eps, x - t)[
            :, 0
        ].mean() / (1 - alpha)
    assert weights.ndim == 1 and weights.shape[0] == x.shape[0]
    return t + smooth_max_function(smoother_type, eps, x - t)[:, 0].dot(
        weights
    ) / (1 - alpha)
...
def smooth_conditional_value_at_risk_gradient_split(
    ...
    grad[-1] = 1 - smooth_max_function_first_derivative(
        smoother_type, eps, x - t
    ).squeeze().dot(weights) / ((1 - alpha))
    grad[:-1] = (
...
def smooth_conditional_value_at_risk_composition_split(
    ...
    obj_val = t + smooth_max_function(smoother_type, eps, fun_vals - t).dot(
        weights
    ) / (1 - alpha)

    grad = np.empty(x.shape[0] + 1)
    grad[-1] = 1 - smooth_max_function_first_derivative(
        smoother_type, eps, fun_vals - t
    ).dot(weights) / ((1 - alpha))
    jac_vals = jac(x)
    assert jac_vals.shape == (nsamples, x.shape[0])
    grad[:-1] = (
        smooth_max_function_first_derivative(smoother_type, eps, fun_vals - t)[
            :, None
        ]
        * jac_vals
        / (1 - alpha)
...
def cvar_regression_quadrature(
    basis_matrix,
    values,
    alpha,
    nquad_intervals,
    ...
    if not trapezoid_rule:
        ...
        beta = np.linspace(alpha, 1, nquad_intervals + 2)[
            :-1
        ]  # quadrature points
        dx = beta[1] - beta[0]
    ...
    v_coef = weights / (1 - beta) * 1.0 / nsamples * 1 / (1 - alpha)
    ...
    if not trapezoid_rule:
        ...
        constraints_1b_J = (
            np.repeat(np.arange(nquad_intervals + 1), nsamples) + nbasis
        )
        constraints_1b_data = -np.repeat(
            np.ones(nquad_intervals + 1), nsamples
        )
        ...
        constraints_3 = np.hstack(
            (
                np.zeros((nvconstraints, nbasis + nquad_intervals)),
                -Iv,
                np.zeros((nvconstraints, 1)),
            )
        )
    ...
    sol = np.asarray(solvers.lp(c=c, G=G, h=h, solver=solver_name)["x"])[
        :nbasis
    ]
...
def cvar_regression(basis_matrix, values, alpha, verbosity=1):
    ...
    active_index = (
        int(np.ceil(alpha * nsamples)) - 1
    )  # 0 based index 0,...,nsamples-1
    ...
    c_arr = np.hstack(
        (
            ...
            1.0 / (nsamples * (1 - alpha)) * np.ones(1),
        )
    )
