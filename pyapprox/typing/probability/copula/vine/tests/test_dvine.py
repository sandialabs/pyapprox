...
def _make_gaussian_copula(
    partial_corrs: Dict[int, List[float]],
    nvars: int,
    bkd: Backend[Array],
) -> GaussianCopula[Array]:
...
def _standard_normal_invcdf(u: Array, bkd: Backend[Array]) -> Array:
...
def _empirical_correlation(z: Array, bkd: Backend[Array]) -> Array:
...
class TestDVineCopulaBase(Generic[Array], unittest.TestCase):
    ...
    def test_constructor_validation_wrong_count(self) -> None:
        """Wrong number of copulas at a tree level raises ValueError."""
        pc = {
            1: [BivariateGaussianCopula(0.5, self._bkd)],  # need 2
        }
        with self.assertRaises(ValueError):
            DVineCopula(pc, 3, 1, self._bkd)

    def test_constructor_validation_invalid_truncation(self) -> None:
        """Truncation level > nvars - 1 raises ValueError."""
        pc = {
            1: [BivariateGaussianCopula(0.5, self._bkd)],
        }
        with self.assertRaises(ValueError):
            DVineCopula(pc, 2, 5, self._bkd)

    def test_constructor_validation_negative_truncation(self) -> None:
        """Negative truncation level raises ValueError."""
        with self.assertRaises(ValueError):
            DVineCopula({}, 3, -1, self._bkd)

    # -- Property methods --

    def test_nvars_nparams(self) -> None:
        ...
        self.assertEqual(dvine.nvars(), 3)
        self.assertEqual(dvine.nparams(), 3)
        self.assertEqual(dvine.npair_copulas(), 3)
        self.assertEqual(dvine.truncation_level(), 2)

    def test_pair_copula_access(self) -> None:
        """Retrieve specific pair copula by tree and edge."""
        partial_corrs = {1: [0.6, 0.5], 2: [0.3]}
        dvine = _make_gaussian_dvine(partial_corrs, 3, self._bkd)
        pc = dvine.pair_copula(1, 0)
        self.assertIsInstance(pc, BivariateGaussianCopula)
    ...
    def test_logpdf_independence_is_zero(self) -> None:
    ...
    def test_logpdf_bivariate_matches(self) -> None:
    ...
    def test_logpdf_3var_gaussian_matches(self) -> None:
    ...
    def test_logpdf_4var_gaussian_matches(self) -> None:
    ...
    def test_logpdf_4var_matches_mvn_density(self) -> None:
        ...
        z = _standard_normal_invcdf(u_clipped, self._bkd)
    ...
    def test_logpdf_truncated_vs_full(self) -> None:
        ...
        dvine_full = _make_gaussian_dvine(partial_corrs_full, 4, self._bkd)
        ...
        diff = self._bkd.abs(logpdf_full - logpdf_trunc)
        max_diff = self._bkd.max(diff)
        self.assertGreater(float(self._bkd.to_numpy(max_diff)), 0.01)
    ...
    def test_sample_shape_and_range(self) -> None:
        ...
        # All values in (0, 1)
        min_val = self._bkd.min(samples)
        max_val = self._bkd.max(samples)
        self.assertGreater(float(self._bkd.to_numpy(min_val)), 0.0)
        self.assertLess(float(self._bkd.to_numpy(max_val)), 1.0)

    def test_sample_correlation_matches(self) -> None:
        ...
        u_clipped = self._bkd.clip(samples, 1e-10, 1.0 - 1e-10)
        z = _standard_normal_invcdf(u_clipped, self._bkd)

        empirical_corr = _empirical_correlation(z, self._bkd)

        self._bkd.assert_allclose(empirical_corr, R, atol=0.03)

    def test_sample_logpdf_recovers_mvn_density(self) -> None:
    ...
    def test_sample_gaussian_statistics(self) -> None:
        ...
        self._bkd.assert_allclose(empirical_cov, R, atol=0.03)

    # -- Precision / correlation matrix tests --

    def test_from_precision_matrix_roundtrip_tridiag(self) -> None:
    ...
    def test_from_precision_matrix_correlation_recovery(self) -> None:
    ...
    def test_from_precision_matrix_logpdf_matches_mvn(self) -> None:
    ...
    def test_to_correlation_matrix_gaussian(self) -> None:
        ...
        self._bkd.assert_allclose(R_actual, R_expected, rtol=1e-12)

    def test_to_correlation_matrix_rejects_non_gaussian(self) -> None:
        ...
        dvine = DVineCopula(pc, 2, 1, self._bkd)
        with self.assertRaises(TypeError):
            dvine.to_correlation_matrix()


class TestDVineCopulaNumpy(TestDVineCopulaBase[NDArray[Any]]):
    __test__ = True
...
class TestDVineCopulaTorch(TestDVineCopulaBase[torch.Tensor]):
    __test__ = True
