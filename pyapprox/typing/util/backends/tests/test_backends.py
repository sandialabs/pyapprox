...
class TestBackend(Generic[Array]):
    ...
    def test_2d_array(self) -> None:
        ...
        array_2d = backend.array(
            [[1, 2], [3, 4]], dtype=backend.double_dtype()
        )
        result = foo(array_2d, backend)
        expected = array_2d  # Identity matrix dot product should return the original array
    ...
    def test_1d_array(self) -> None:
        ...
        expected = array_1d  # Identity matrix dot product should return the original array
        backend.assert_allclose(result, expected)
