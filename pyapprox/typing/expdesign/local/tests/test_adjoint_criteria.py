...
class TestCOptimalCriterionNumpy(TestCOptimalCriterion[NDArray[Any]]):
    ...
    def test_legacy_comparison(self):
        ...
        from pyapprox.expdesign.local import COptimalLstSqCriterion
        from pyapprox.util.backends.numpy import NumpyMixin

        dm = LeastSquaresDesignMatrices(
            self._design_factors, self._bkd, self._noise_mult
        )
        new_crit = COptimalCriterion(dm, self._vec, self._bkd)
        legacy_crit = COptimalLstSqCriterion(
...
class TestAOptimalCriterionNumpy(TestAOptimalCriterion[NDArray[Any]]):
    ...
    def test_legacy_comparison(self):
        """Compare with legacy implementation."""
        from pyapprox.expdesign.local import AOptimalLstSqCriterion
        from pyapprox.util.backends.numpy import NumpyMixin

        dm = LeastSquaresDesignMatrices(
            self._design_factors, self._bkd, self._noise_mult
        )
        new_crit = AOptimalCriterion(dm, self._bkd)
        legacy_crit = AOptimalLstSqCriterion(
            self._design_factors,
            noise_mult=self._noise_mult,
            backend=NumpyMixin,
        )
...
class TestIOptimalCriterionNumpy(TestIOptimalCriterion[NDArray[Any]]):
    ...
    def test_legacy_comparison(self):
        """Compare with legacy implementation."""
        from pyapprox.expdesign.local import IOptimalLstSqCriterion
        from pyapprox.util.backends.numpy import NumpyMixin

        dm = LeastSquaresDesignMatrices(
            self._design_factors, self._bkd, self._noise_mult
        )
        new_crit = IOptimalCriterion(dm, self._pred_factors, self._bkd)
        legacy_crit = IOptimalLstSqCriterion(
            self._design_factors,
            self._pred_factors,
            noise_mult=self._noise_mult,
            backend=NumpyMixin,
