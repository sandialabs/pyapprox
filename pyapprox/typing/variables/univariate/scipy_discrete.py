...
class DiscreteScipyRandomVariable1D(Generic[Array]):
    ...
    def __init__(
        self,
        scipy_rv: Any,  # Expected to be a ScipyDiscreteFrozenProtocol
        bkd: Backend[Array],
    ):
    ...
    def _validate_marginal(self, scipy_rv: Any) -> None:
        """
        Validate that the marginal is a discrete SciPy random variable.
        ...
        ------
        ValueError
            If the marginal is not a discrete SciPy random variable.
        """
        if scipy_rv.dist.name not in _discrete_distns._distn_names:  # type: ignore
    ...
    def __call__(self, samples: Array) -> Array:
        ...
        ----------
        samples : Array
            The points at which to evaluate the PMF.

        Returns
        -------
        pmf_vals : Array
            The evaluated PMF values.
        """
    ...
    def is_bounded(self) -> bool:
        """
        Check if the variable is bounded and discrete.
