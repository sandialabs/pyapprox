...
for _ in range(15):
    ...
    cand_arr = fitter_anim._index_gen.get_candidate_indices()
...
all_betas = [s[0][0].max() for s in snapshots]
all_betas += [s[1][0].max() for s in snapshots if s[1].shape[1] > 0]
max_beta = max(all_betas) if all_betas else 3
...
def animate(ii):
    ...
    # Left: multi-index set — candidates first (behind), then selected
    # Draw candidates (dashed outline, lighter fill, colored by config level)
    for j in range(cand_idx.shape[1]):
        beta, alpha = cand_idx[0, j], cand_idx[1, j]
        color = "#93C5E8" if alpha == 0 else "#F5A9A9"
        rect = plt.Rectangle(
            (beta - 0.4, alpha - 0.3), 0.8, 0.6,
    ...
    # Draw selected (solid fill, colored by config level)
...
def _run_steps(fitter_obj, factory_obj, cost_model_obj, max_steps=15):
    ...
    for _ in range(max_steps):
        ...
        cand_arr = fitter_obj._index_gen.get_candidate_indices()
        cand = (bkd.to_numpy(cand_arr).astype(int)
                if cand_arr is not None else np.zeros((2, 0), dtype=int))
        approx = bkd.to_numpy(res.surrogate(z_eval)).ravel()
        err = fitter_obj.current_error()

        # Cumulative cost = sum over configs of n_samples * cost_per_sample
        cum_cost = 0.0
        for cfg, tracker in fitter_obj._trackers.items():
            cum_cost += tracker.n_unique_samples() * cost_model_obj(cfg)
...
all_betas = []
for snaps in [snaps_uw, snaps_cw]:
    for s in snaps:
        all_betas.append(s[0][0].max())
        if s[1].shape[1] > 0:
            all_betas.append(s[1][0].max())
max_beta = max(all_betas) if all_betas else 3

fig, axes = plt.subplots(1, 2, figsize=(14, 4.5))

def _draw_indices(ax, sel, cand, title):
    """Draw selected + candidate boxes on ax."""
    # Candidates (dashed, behind)
    for j in range(cand.shape[1]):
        beta, alpha = cand[0, j], cand[1, j]
        color = "#93C5E8" if alpha == 0 else "#F5A9A9"
        rect = plt.Rectangle(
            (beta - 0.4, alpha - 0.3), 0.8, 0.6,
            facecolor=color, edgecolor="black", alpha=0.4,
        ...
        ax.add_patch(rect)
        ax.text(beta, alpha, f"({beta},{alpha})",
                ha="center", va="center", fontsize=8, color="gray")
    # Selected (solid, on top)
    for j in range(sel.shape[1]):
        beta, alpha = sel[0, j], sel[1, j]
        color = "#2C7FB8" if alpha == 0 else "#E74C3C"
        rect = plt.Rectangle(
            (beta - 0.4, alpha - 0.3), 0.8, 0.6,
            facecolor=color, edgecolor="black", alpha=0.7, linewidth=1.5,
        ...
        ax.text(beta, alpha, f"({beta},{alpha})",
                ha="center", va="center", fontsize=9)
    ax.set_xlim(-0.6, max_beta + 1)
    ax.set_ylim(-0.5, 2)
    ax.set_xlabel("Physical level $\\beta$", fontsize=11)
    ax.set_ylabel("Config level $\\alpha$", fontsize=11)
