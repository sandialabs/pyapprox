...
for level in range(5):
    ...
    print(
        f"Level {level}: {result.surrogate.nsubspaces():>3} subspaces, "
        f"{nsamples:>4} samples"
    )
```
...
nsamples = samples.shape[1]
print(f"Sparse grid (level {level}, D={nvars}): {nsamples} points")
...
for ell in range(max_ell + 1):
    tp_2d = growth(ell) ** 2
    fac2 = TensorProductSubspaceFactory(bkd, [leja_factory] * 2, growth)
    sg_2d = IsotropicSparseGridFitter(bkd, fac2, ell).get_samples().shape[1]
    tp_5d = growth(ell) ** 5
    fac5 = TensorProductSubspaceFactory(bkd, [leja_factory] * 5, growth)
    sg_5d = IsotropicSparseGridFitter(bkd, fac5, ell).get_samples().shape[1]
    tp_10d = growth(ell) ** 10
    fac10 = TensorProductSubspaceFactory(bkd, [leja_factory] * 10, growth)
    sg_10d = IsotropicSparseGridFitter(bkd, fac10, ell).get_samples().shape[1]
    print(
...
for level in range(5):
    ...
    print(f"SG level {level}: {samples.shape[1]:>5} pts, error = {err:.2e}")
...
print(f"Sparse grid mean (level 4, {nsamples_quad} pts): {sg_mean:.8f}")
