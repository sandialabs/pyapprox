...
for row, cfg in enumerate(configs):
    ...
    # Trace plot (xi_1 only for clarity)
    ax_trace = axes[row, 0]
    ...
    ax_trace.set_title(f'{cfg["label"]} (accept rate: {acc_rate:.0%})',
...
sigma_multipliers = np.logspace(-1.5, 1.2, 20)
base_sigma = np.array([0.5, 0.5])
acc_rates = []
ess_values = []

for mult in sigma_multipliers:
    ch, ar = run_mh(3000, base_sigma * mult, seed=123)
    acc_rates.append(ar)
...
chain_demo, _ = run_mh(3000, np.array([0.5, 0.5]), seed=7)
burnin_val = 500

fig, ax = plt.subplots(figsize=(8, 4.5))

# Use the same bins for both histograms so they overlay cleanly
bins = np.linspace(chain_demo[:, 0].min(), chain_demo[:, 0].max(), 50)

ax.hist(chain_demo[:, 0], bins=bins, density=True, color="#C0392B",
        alpha=0.35, edgecolor="#C0392B", lw=0.5, label="All samples (with burn-in)")
ax.hist(chain_demo[burnin_val:, 0], bins=bins, density=True, color="#2C7FB8",
        alpha=0.45, edgecolor="#2C7FB8", lw=0.5, label="After burn-in removed")
...
for cfg in configs:
    chain_acf, _ = run_mh(5000, cfg["sigma"], seed=99)
    x = chain_acf[500:, 0]
    x = x - np.mean(x)
...
# 1D posterior on a grid for visualization
...
xi_current = 0.3
xi_prop1 = 2.5   # large step, will be rejected
xi_prop2 = 0.8   # smaller step, will be accepted
...
for ax, xi_prop, p_prop, stage, color, outcome in [
    (ax1, xi_prop1, p_prop1, "Stage 1 (large step)", "#C0392B", "Rejected ✗"),
    (ax2, xi_prop2, p_prop2, "Stage 2 (smaller step)", "#27AE60", "Accepted ✓"),
]:
    ax.fill_between(xi_grid_1d, post_vals_1d, alpha=0.12, color="#E67E22")
    ...
    ax.annotate("", xy=(xi_prop, y_arrow), xytext=(xi_current, y_arrow),
                arrowprops=dict(arrowstyle="->", lw=2, color=color))

    # Proposal distribution sketch
    sigma_sketch = 2.0 if "Stage 1" in stage else 0.6
    prop_sketch = sp_norm.pdf(xi_grid_1d, xi_current, sigma_sketch)
    prop_sketch = prop_sketch / prop_sketch.max() * post_vals_1d.max() * 0.2
    ...
    ax.text(0.98, 0.85, f"{stage}\n{outcome}", transform=ax.transAxes,
            ...
            bbox=dict(boxstyle="round,pad=0.3",
                      fc="#f5d5d5" if "Rejected" in outcome else "#d5f5d5",
                      ec="gray", lw=0.5))
...
def plot_proposal_ellipse(ax, center, cov, color, label, scale=2):
    eigvals, eigvecs = np.linalg.eigh(cov)
    angle = np.degrees(np.arctan2(eigvecs[1, 1], eigvecs[0, 1]))
    w = 2 * scale * np.sqrt(eigvals[1])
    h = 2 * scale * np.sqrt(eigvals[0])
    ell = Ellipse(xy=center, width=w, height=h, angle=angle,
...
chain_am = bkd.to_numpy(result_am.samples).T  # (n_steps, 2)
...
for ax, title, chain_slice, cov_ell, c_color, ell_label in [
    ...
    plot_proposal_ellipse(ax, center, cov_ell, c_color, ell_label)
...
```{python}
#| echo: false
#| fig-cap: "Comparison of four MCMC variants on the KLE beam problem (5,000 steps each). Top row: trace plots of $\\xi_1$. Bottom row: 2D scatter of posterior samples with ESS annotations. Focus on ESS rather than acceptance rate — DR achieves 90% acceptance but the lowest ESS, because most accepted moves are tiny second-stage steps that barely advance the chain."
...
for col, (name, ch, acc, color) in enumerate(methods):
    ...
    ax_t.set_ylabel(r"$\xi_1$", fontsize=10) if col == 0 else None
    ...
    ax_s.scatter(xi1_true, xi2_true, marker="D", s=50, color="k", zorder=5)
    ax_s.set_xlabel(r"$\xi_1$", fontsize=10)
    ax_s.set_ylabel(r"$\xi_2$", fontsize=10) if col == 0 else None
    ax_s.grid(True, alpha=0.15)

    # ESS annotation
    ess = effective_sample_size(ch_post[:, 0])
    ax_s.text(0.05, 0.92, f"ESS = {ess:.0f}", transform=ax_s.transAxes,
...
Each MCMC step requires one model evaluation. For this 1D cantilever beam with a fast FEM solver, 5,000 evaluations are trivial. But for a 3D FEM model that takes minutes per evaluation, 5,000 evaluations is prohibitive. The [surrogate-accelerated inference](surrogate_inference.qmd) tutorial shows how to replace the expensive model with a PCE inside the MCMC loop, reducing the cost by orders of magnitude.
:::
...
- [Model Discrepancy](model_discrepancy.qmd) --- Accounting for model error in Bayesian inference
