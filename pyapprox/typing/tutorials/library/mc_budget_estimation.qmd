...
for col, ax in enumerate(axes):
    ax.hist(all_means[:, col], bins=30, density=True, color=colors[col],
    ...
    x_grid = np.linspace(all_means[:, col].min(), all_means[:, col].max(), 200)
...
There is no universal rule for $M_{\text{pilot}}$, but some heuristics:

- **For the mean only**: the pilot needs to estimate $\boldsymbol{\Sigma}$ (second moments). A pilot of $M_{\text{pilot}} \approx 30\text{--}50$ is often adequate.
- **For the variance**: the pilot needs to estimate $\mathbf{W}$ (fourth moments), which converges more slowly. A pilot of $M_{\text{pilot}} \approx 100\text{--}200$ is safer.
- **When in doubt**: use the pilot samples as the first batch of the final experiment. The total budget is $M_{\text{pilot}} + M_{\text{additional}}$, so pilot samples are never wasted.
:::
...
ax1.set_title(f"Estimator covariance (target_cost = {target_cost:.0f})", fontsize=11)
fig.colorbar(im1, ax=ax1, shrink=0.7)

# Bar chart of diagonal
diag_vals = np.diag(pred_cov_plot)
colors_bar = ["#2C7FB8"] * nqoi + ["#E67E22"] * (nstats - nqoi)
ax2.bar(range(nstats), diag_vals, color=colors_bar, edgecolor="k", lw=0.3)
...
- [Multi-Fidelity Monte Carlo](multi_fidelity.qmd) --- Using cheap surrogate models to reduce the constant in the $1/M$ rate
- [Surrogate Models](surrogate_models.qmd) --- Replacing the expensive model with a polynomial or Gaussian process approximation
