...
class TestLeastSquares(Generic[Array], unittest.TestCase):
    ...
    def _create_aetc(self, nmodels: int = 3) -> AETC[Array]:
        ...
        def dummy_rvs(nsamples: int) -> Array:
            return self._bkd.asarray(np.random.randn(1, nsamples))

        models = [dummy_model] * nmodels
        costs = self._bkd.asarray([10.0 ** (-i) for i in range(nmodels)])

        return AETC(models, dummy_rvs, costs, oracle_stats=None, bkd=self._bkd)

    def test_least_squares_coefficients_shape(self) -> None:
        """Test that _least_squares returns correct shapes."""
        nsamples = 50
        ncovariates = 2

        # Typing convention: (nqoi, nsamples) and (ncovariates, nsamples)
        hf_values = self._bkd.asarray(np.random.randn(1, nsamples))
        covariate_values = self._bkd.asarray(np.random.randn(ncovariates, nsamples))
    ...
    def test_least_squares_perfect_fit(self) -> None:
        ...
        x1 = self._bkd.asarray(np.linspace(-1, 1, nsamples).reshape(1, -1))
        # Typing convention: (nqoi, nsamples)
    ...
    def test_least_squares_design_matrix_first_column(self) -> None:
        ...
        expected_ones = self._bkd.ones((nsamples,))
        self._bkd.assert_allclose(X[:, 0], expected_ones, rtol=1e-12)

        # Remaining columns should match covariate_values transposed
        # Input is (ncovariates, nsamples), X stores (nsamples, ncovariates)
        self._bkd.assert_allclose(X[:, 1:], covariate_values.T, rtol=1e-12)


class TestLeastSquaresNumpy(TestLeastSquares[NDArray[Any]]):
    """NumPy backend tests."""
...
class TestLeastSquaresTorch(TestLeastSquares[torch.Tensor]):
...
class TestSubsetOracleStats(Generic[Array], unittest.TestCase):
    ...
    def test_subset_oracle_stats_shapes(self) -> None:
    ...
    def test_subset_oracle_stats_x_Sp_structure(self) -> None:
        ...
        cov = self._bkd.eye(nmodels)
        means = self._bkd.asarray([[0.0], [1.5], [2.5]])
        ...
        self._bkd.assert_allclose(x_Sp, expected_x_Sp, rtol=1e-12)
    ...
    def test_subset_oracle_stats_single_covariate(self) -> None:
        ...
        expected_x_Sp = self._bkd.asarray([[1.0], [2.0]])
        self._bkd.assert_allclose(x_Sp, expected_x_Sp, rtol=1e-12)


class TestSubsetOracleStatsNumpy(TestSubsetOracleStats[NDArray[Any]]):
    """NumPy backend tests."""
...
class TestSubsetOracleStatsTorch(TestSubsetOracleStats[torch.Tensor]):
    """PyTorch backend tests."""
...
class TestAllocateSamples(Generic[Array], unittest.TestCase):
    ...
    def _create_aetc_with_k2(self, nmodels: int = 3) -> AETC[Array]:
        """Create an AETC instance with a mock _find_k2 method."""

        class MockAETC(AETC[Array]):
            def _find_k2(
                self,
                beta_Sp: Array,
                Sigma_S: Array,
                ...
                nmodels = len(costs_S)
                k2 = bkd.asarray(0.5)
                nsamples_per_subset = bkd.ones((nmodels,)) / nmodels
        ...
        return MockAETC(models, dummy_rvs, costs, oracle_stats=None, bkd=self._bkd)

    def test_allocate_samples_returns_tuple(self) -> None:
        """Test that _allocate_samples returns (k1, k2, nsamples_per_subset)."""
        nmodels = 3

        # Create inputs
        beta_Sp = self._bkd.asarray([[1.0], [0.5], [0.3]])
        Sigma_S = self._bkd.eye(2) * 0.5
        sigma_S_sq = self._bkd.asarray(0.1)
        x_Sp = self._bkd.asarray([[1.0], [0.5], [0.3]])
        Lambda_Sp = self._bkd.eye(3)
        costs_S = self._bkd.asarray([0.1, 0.01])
        exploit_budget = self._bkd.asarray(100.0)

        aetc = self._create_aetc_with_k2(nmodels)
    ...
    def test_allocate_samples_single_covariate(self) -> None:
        ...
        beta_Sp = self._bkd.asarray([[1.0], [0.8]])
        Sigma_S = self._bkd.asarray([[0.5]])  # 1x1 covariance
        sigma_S_sq = self._bkd.asarray(0.1)
        x_Sp = self._bkd.asarray([[1.0], [0.5]])
        Lambda_Sp = self._bkd.eye(2)
        costs_S = self._bkd.asarray([0.1])  # Single LF model cost
        exploit_budget = self._bkd.asarray(100.0)
    ...
    def test_allocate_samples_k1_formula(self) -> None:
        ...
        x_Sp = self._bkd.asarray([[1.0], [0.5]])
        Lambda_Sp = self._bkd.eye(2)
        sigma_S_sq = self._bkd.asarray(0.25)
        ...
        beta_Sp = self._bkd.asarray([[1.0], [0.5]])
        Sigma_S = self._bkd.asarray([[0.5]])
        costs_S = self._bkd.asarray([0.1])
        exploit_budget = self._bkd.asarray(100.0)

        aetc = self._create_aetc_with_k2(2)
...
class TestAllocateSamplesNumpy(TestAllocateSamples[NDArray[Any]]):
    """NumPy backend tests."""
...
class TestAllocateSamplesTorch(TestAllocateSamples[torch.Tensor]):
    """PyTorch backend tests."""
...
class TestValidateSubsets(Generic[Array], unittest.TestCase):
    ...
    def test_validate_subsets_none_generates_all(self) -> None:
    ...
    def test_validate_subsets_rejects_duplicates(self) -> None:
        ...
        subsets = [self._bkd.asarray([0, 0], dtype=int)]

        with self.assertRaises(ValueError):
            aetc._validate_subsets(subsets)

    def test_validate_subsets_rejects_out_of_range(self) -> None:
        """Test that out-of-range indices are rejected."""
        nmodels = 3  # LF models are 0, 1 (max index is 1)
        aetc = self._create_aetc(nmodels)

        subsets = [self._bkd.asarray([2], dtype=int)]  # Index 2 is out of range

        with self.assertRaises(ValueError):
            aetc._validate_subsets(subsets)


class TestValidateSubsetsNumpy(TestValidateSubsets[NDArray[Any]]):
    """NumPy backend tests."""
...
class TestValidateSubsetsTorch(TestValidateSubsets[torch.Tensor]):
    """PyTorch backend tests."""
...
class TestAETCBLUEFindK2(Generic[Array], unittest.TestCase):
    ...
    def _create_aetcblue(self, nmodels: int = 3) -> AETCBLUE[Array]:
        ...
        return AETCBLUE(models, dummy_rvs, costs, oracle_stats=None, bkd=self._bkd)

    def test_find_k2_returns_tuple(self) -> None:
        """Test that _find_k2 returns (k2, nsamples_per_subset)."""
        aetcblue = self._create_aetcblue(nmodels=3)
        ...
        self.assertTrue(k2.ndim == 0 or (k2.ndim == 1 and k2.shape[0] == 1))
...
class TestAETCBLUEFindK2Torch(TestAETCBLUEFindK2[torch.Tensor]):
    """PyTorch backend tests."""
...
class TestAETCBLUEExploitation(Generic[Array], unittest.TestCase):
    ...
    def _create_mock_explore_result(self):
        ...
        subset = self._bkd.asarray([0, 1], dtype=int)
        subset_cost = 0.11
        beta_Sp = self._bkd.asarray([[1.5], [0.8], [0.6]])
        sigma_S = self._bkd.asarray([[1.0, 0.3], [0.3, 1.0]])
        rounded_nsamples_per_subset = self._bkd.asarray([10.0, 20.0, 5.0])
        nsamples_per_subset = self._bkd.asarray([10.2, 19.8, 5.1])
        loss = 0.05
        k1 = 0.1
        BLUE_variance = 0.02
        exploit_budget = 50.0
    ...
    def test_find_exploit_mean_uses_correct_reg_blue(self) -> None:
        ...
        nmodels = 3
        aetcblue = self._create_aetcblue(nmodels)
        ...
        subset = result[1]
        costs = self._bkd.asarray([1.0, 0.1, 0.01])
        ...
        np.random.seed(123)
        values_per_model = [
            self._bkd.asarray(np.random.randn(1, int(nsamples_per_model[i])))
            for i in range(len(self._bkd.to_numpy(subset)))
        ]
...
class TestAETCBLUEExploitationNumpy(TestAETCBLUEExploitation[NDArray[Any]]):
    """NumPy backend tests."""
...
class TestAETCBLUEExploitationTorch(TestAETCBLUEExploitation[torch.Tensor]):
    """PyTorch backend tests."""
...
class TestAETCMC(Generic[Array], unittest.TestCase):
    """Test AETCMC methods."""
    ...
    def _create_aetcmc(self, nmodels: int = 3) -> AETCMC[Array]:
        ...
        return AETCMC(models, dummy_rvs, costs, oracle_stats=None, bkd=self._bkd)
    ...
    def test_find_k2_formula(self) -> None:
        ...
        expected_k2 = exploit_cost * self._bkd.trace(
    ...
    def test_explore_result_to_dict_keys(self) -> None:
        ...
        result = (
            ...
            self._bkd.asarray([0, 1], dtype=int),  # subset
            0.11,  # subset_cost
            self._bkd.asarray([[1.5], [0.8], [0.6]]),  # beta_Sp
            self._bkd.asarray([[1.0, 0.3], [0.3, 1.0]]),  # sigma_S
            self._bkd.asarray([10.0, 20.0, 5.0]),  # rounded_nsamples_per_subset
            self._bkd.asarray([10.2, 19.8, 5.1]),  # nsamples_per_subset
            0.05,  # loss
            0.1,  # k1
            0.02,  # BLUE_variance
            50.0,  # exploit_budget
            self._bkd.asarray([0.1, 0.01]),  # mlblue_subset_costs
        )
...
class TestAETCMCNumpy(TestAETCMC[NDArray[Any]]):
    """NumPy backend tests."""
...
class TestAETCMCTorch(TestAETCMC[torch.Tensor]):
    """PyTorch backend tests."""
...
class TestExploreStep(Generic[Array], unittest.TestCase):
    ...
    def test_explore_result_structure(self) -> None:
        ...
        self.assertEqual(len(beta_Sp.shape), 2)

        # sigma_S should be 2D square
        self.assertEqual(len(sigma_S.shape), 2)
        self.assertEqual(sigma_S.shape[0], sigma_S.shape[1])


class TestExploreStepNumpy(TestExploreStep[NDArray[Any]]):
    """NumPy backend tests."""
...
class TestExploreStepTorch(TestExploreStep[torch.Tensor]):
    """PyTorch backend tests."""
...
class TestExploitProducesMean(Generic[Array], unittest.TestCase):
    ...
    def _create_aetcblue(self) -> AETCBLUE[Array]:
        """Create an AETCBLUE instance for testing."""
        from pyapprox.typing.benchmarks.functions.multifidelity import (
            TunableModelEnsemble,
        )
    ...
    def test_exploit_mean_is_finite(self) -> None:
        ...
        self.assertTrue(np.isfinite(float(mean)))


class TestExploitProducesMeanNumpy(TestExploitProducesMean[NDArray[Any]]):
    """NumPy backend tests."""
...
class TestExploitProducesMeanTorch(TestExploitProducesMean[torch.Tensor]):
    """PyTorch backend tests."""
...
class TestFullEstimatePipeline(Generic[Array], unittest.TestCase):
    ...
    def test_blue_variance_formula(self) -> None:
        ...
        subset = result["subset"] + 1
        subset_np = self._bkd.to_numpy(subset).astype(int)
        costs = self._bkd.asarray([10.0 ** (-i) for i in range(3)])
        ...
        computed_var_scalar = float(self._bkd.to_numpy(computed_var).ravel()[0])
        self._bkd.assert_allclose(
...
class TestFullEstimatePipelineNumpy(TestFullEstimatePipeline[NDArray[Any]]):
    """NumPy backend tests."""
...
class TestFullEstimatePipelineTorch(TestFullEstimatePipeline[torch.Tensor]):
    """PyTorch backend tests."""
...
class TestOptimalLossOracleVsMC(unittest.TestCase):
