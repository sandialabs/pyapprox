...
class TestFiniteDifferenceJacobian(Generic[Array], unittest.TestCase):
    ...
    def test_forward_difference_quadratic(self) -> None:
        ...
        model = FunctionFromCallable(
            nqoi=1, nvars=1, fun=fun, bkd=self._bkd
        )
    ...
    def test_centered_more_accurate_than_forward(self) -> None:
        ...
        self.assertTrue(
            self._bkd.all_bool(error_centered < error_forward)
        )
    ...
    def test_multi_output_jacobian(self) -> None:
        ...
        model = FunctionFromCallable(
            nqoi=2, nvars=2, fun=fun, bkd=self._bkd
        )
        fd_model = FiniteDifferenceWrapper(model, method="centered")
...
class TestFiniteDifferenceHessian(Generic[Array], unittest.TestCase):
    ...
    def test_hessian_requires_nqoi_one(self) -> None:
        ...
        model = FunctionFromCallable(
            nqoi=2, nvars=1, fun=fun, bkd=self._bkd
        )
        fd_model = FiniteDifferenceWrapper(model, method="centered")
...
class TestFiniteDifferencePassthrough(Generic[Array], unittest.TestCase):
    ...
    def test_nvars_nqoi_passthrough(self) -> None:
        ...
        model = FunctionFromCallable(
            nqoi=1, nvars=3, fun=fun, bkd=self._bkd
        )
        fd_model = FiniteDifferenceWrapper(model)
...
class TestFiniteDifferenceJacobianNumpy(
    TestFiniteDifferenceJacobian[NDArray[Any]]
):
    def setUp(self) -> None:
        self._bkd = NumpyBkd()
        super().setUp()
...
class TestFiniteDifferenceHessianNumpy(
    TestFiniteDifferenceHessian[NDArray[Any]]
):
    def setUp(self) -> None:
        self._bkd = NumpyBkd()
        super().setUp()
...
class TestFiniteDifferenceJacobianTorch(
    TestFiniteDifferenceJacobian[torch.Tensor]
):
    def setUp(self) -> None:
...
class TestFiniteDifferenceHessianTorch(
    TestFiniteDifferenceHessian[torch.Tensor]
):
    def setUp(self) -> None:
        torch.set_default_dtype(torch.float64)
        self._bkd = TorchBkd()
