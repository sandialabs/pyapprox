...
from pyapprox.typing.inverse.sampling.tests.test_distributions import (
    BananaLogPosterior,
...
class TestMCMCPosteriorBase(Generic[Array], unittest.TestCase):
    ...
    def test_correlated_gaussian_metropolis(self) -> None:
        """Test Metropolis-Hastings on correlated Gaussian target."""
        np.random.seed(42)

        mean = self.bkd().asarray(np.array([1.0, -1.0]))
        cov = self.bkd().asarray(np.array([[1.0, 0.5], [0.5, 2.0]]))
        target = CorrelatedGaussianLogPosterior(self.bkd(), mean, cov)
    ...
    def test_correlated_gaussian_adaptive(self) -> None:
        ...
        mean = self.bkd().asarray(np.array([0.5, 0.5]))
    ...
    def test_correlated_gaussian_hmc(self) -> None:
        ...
        cov = self.bkd().asarray(np.array([[1.0, 0.8], [0.8, 1.0]]))
        target = CorrelatedGaussianLogPosterior(self.bkd(), mean, cov)
    ...
    def test_correlated_gaussian_dram(self) -> None:
        ...
        mean = self.bkd().asarray(np.array([1.0, 1.0]))
        cov = self.bkd().asarray(np.array([[2.0, 0.5], [0.5, 1.0]]))
        target = CorrelatedGaussianLogPosterior(self.bkd(), mean, cov)
    ...
    def test_banana_distribution_dram(self) -> None:
        ...
        self.assertLess(np.abs(sample_mean[0] - true_mean[0]), 0.3)

    def test_gaussian_mixture_adaptive(self) -> None:
    ...
    def test_diagnostics_on_converged_chain(self) -> None:
        ...
        mean = self.bkd().asarray(np.array([0.0, 0.0]))
        cov = self.bkd().asarray(np.array([[1.0, 0.0], [0.0, 1.0]]))
        target = CorrelatedGaussianLogPosterior(self.bkd(), mean, cov)
        ...
        ess = effective_sample_size(result.samples, self.bkd())
    ...
    def test_multiple_chains_rhat(self) -> None:
        ...
        # Stack chains
        chains_array = self.bkd().asarray(
            np.stack([self.bkd().to_numpy(c) for c in chains], axis=0)
        )

        # Compute R-hat
        r_hat = rhat(chains_array, self.bkd())
        r_hat_np = self.bkd().to_numpy(r_hat)
...
# NumPy backend tests
class TestMCMCPosteriorNumpy(TestMCMCPosteriorBase[NDArray[Any]]):
    __test__ = True
...
class TestMCMCPosteriorTorch(TestMCMCPosteriorBase[torch.Tensor]):
    __test__ = True
