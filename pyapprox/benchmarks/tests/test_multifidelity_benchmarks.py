...
class TestMFBenchmarks(unittest.TestCase):
    ...
    def _check_benchmark(self, bench):
        ...
        bkd = np
        assert bkd.allclose(
            super(type(bench), bench)._mean(),
            bench.mean())

        assert bkd.allclose(
            super(type(bench), bench)._covariance(),
            bench.covariance())
        ...
        assert bkd.allclose(
            bench.mean(),
            bkd.mean(values, axis=0).reshape(
                bench.nmodels(), bench.nqoi()), rtol=4e-2, atol=1e-3)

        assert bkd.allclose(
            bench.covariance(),
            bkd.cov(values, rowvar=False, ddof=1), rtol=4e-2)

    def test_acv_benchmarks(self):
        cases = [
            PolynomialModelEnsemble(5),
            TunableModelEnsemble(math.pi/4, shifts=[1, 2]),
            MultiOutputModelEnsemble(),
            ShortColumnModelEnsemble(),
        ]
        for case in cases:
