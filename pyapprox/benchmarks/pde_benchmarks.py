...
def gauss_forc_fun(amp, scale, loc, xx):
    ...
    vals = (
        ...
        * torch.exp(
            -torch.sum(
                (torch.as_tensor(xx, dtype=torch.double) - loc) ** 2
                / scale**2,
                axis=0,
            )
        )[:, None]
...
def transient_multi_index_forcing(source1_args, xx, time=0, source2_args=None):
...
def negloglike_functional(
    obs, obs_indices, noise_std, sol, params, ignore_constants=False
):
...
def negloglike_functional_dqdu(
    obs, obs_indices, noise_std, sol, params, ignore_constants=False
):
    if not ignore_constants:
        tmp = 1 / (2 * noise_std**2)
            ...
            flux_vals = [
                (
                    torch.linalg.multi_dot(
                        (mesh._bndry_slice(mesh._dmat(dd), idx, 0), sol)
                    )[:, None]
...
class AdvectionDiffusionReactionKLEModel:
    def __init__(
        ...
        forc_fun,
        functional,
        functional_deriv_funs=[None, None],
        newton_kwargs={},
    ):
        ...
        for ii in range(len(functional_deriv_funs)):
            if (
                functional_deriv_funs[ii] is not None
                and "mesh"
                == inspect.getfullargspec(functional_deriv_funs[ii]).args[0]
            ):
        ...
        if issubclass(type(self._fwd_solver), SteadyStatePDE):
            ...
            dRdp = partial(
                ...
                mesh,
                self._kle,
                self._fwd_solver.physics._bndry_conds,
            )
    ...
    def _set_forward_solver(
        self, mesh, bndry_conds, vel_fun, react_funs, forc_fun
    ):
    ...
    def _set_random_sample(self, sample):
        self._fwd_solver.physics._diff_fun = partial(
            ...
            self._kle(self._kle._bkd.atleast2d(sample[:, None])),
        )
    ...
    def get_num_degrees_of_freedom_cost(self, config_vals):
        if len(config_vals) != self._fwd_solver.physics.mesh.mesh_pts.shape[0]:
    ...
    def __repr__(self):
        return "{0}(kle={1}, mesh={2})".format(
            self.__class__.__name__, self._kle, self._fwd_solver.physics.mesh
        )
...
class TransientAdvectionDiffusionReactionKLEModel(
    ...
    def _get_init_sol(self, sample):
        if self._init_sol is None:
            self._steady_state_fwd_solver.physics._diff_fun = partial(
                self._fast_interpolate, self._kle(sample[:, None])
            )
    ...
    def _eval(self, sample, return_grad=False):
        ...
        sols, times = self._fwd_solver.solve(
            ...
            0,
            self._final_time,
            newton_kwargs=self._newton_kwargs,
            verbosity=0,
        )
    ...
    def get_num_degrees_of_freedom_cost(self, config_vals):
        ...
        assert (
            self._butcher_tableau == "im_beuler1"
            or self._butcher_tableau == "im_crank2"
        )
        return (
            super().get_num_degrees_of_freedom_cost(config_vals[:-1]) * ntsteps
        )
...
def _setup_advection_diffusion_benchmark(
    ...
    bndry_conds = [
        [
            lambda x: torch.full((x.shape[1], 1), alpha * nominal_value),
            "R",
            alpha,
        ],
        [
    ...
    if time_scenario is None:
        ...
        model = AdvectionDiffusionReactionKLEModel(
            ...
            forc_fun,
            functional,
            functional_deriv_funs,
            newton_kwargs,
        )
    else:
        assert time_scenario["init_sol_fun"] is None or callable(
            time_scenario["init_sol_fun"]
        )
        init_sol_fun, final_time, deltat, butcher_tableau = (
            time_scenario["init_sol_fun"],
            time_scenario["final_time"],
            time_scenario["deltat"],
            time_scenario["butcher_tableau"],
        )
        forc_fun = partial(
            transient_multi_index_forcing,
            [amp, scale, loc],
            source2_args=time_scenario["sink"],
        )
...
def _setup_inverse_advection_diffusion_benchmark(
    ...
    if obs_indices is None:
        ...
        obs_indices = np.random.permutation(
            np.delete(np.arange(ndof), bndry_indices)
        )[:nobs]
    ...
    dqdu = partial(
        negloglike_functional_dqdu,
        torch.as_tensor(obs, dtype=torch.double),
        obs_indices,
        noise_std,
    )
    dqdp = partial(
        loglike_functional_dqdp,
        torch.as_tensor(obs, dtype=torch.double),
        obs_indices,
        noise_std,
    )
...
def _setup_multi_index_advection_diffusion_benchmark(
    ...
    return (multi_index_model, variable, config_var_trans, model_ensemble)
...
class DarcyKLE2DModel(SingleSampleModel):
    def __init__(self):
        ...
        bndry_conds = [
            [partial(full_fun_axis_1, 0.0, oned=False), "D"],
            [partial(full_fun_axis_1, 0.0, oned=False), "D"],
            [partial(full_fun_axis_1, 0.0, oned=False), "D"],
            [partial(full_fun_axis_1, 0.0, oned=False), "D"],
        ]
