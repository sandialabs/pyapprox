...
# Should we allow a benchmark to define its own function/model without making it an function instance reusable elsewhere, pros are allows benchmarks where model will likely never be reused, cons function is not reusable without setting up entire benchmark.
...
#TODO: Should benchmarks be defined here on do we allow some to be defined in submodule where most code lives or makes most sense, e.g. pde based benchmark for expdesign lives in expdesign module (not pde) , e.g. there is a benchmarks submodule in expdesign. Decide and document rule, and place in benchmarks.CONVENTIONS.md. Once we decide we must ensure we apply rule consistently, e.g. move bayesian benchmarks to inverse, opt to optimization etc or move all to this module (or follow the split rule if we make one)
