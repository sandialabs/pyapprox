import unittest
import dolfin as dl
from pyapprox.fenics_models.helmholtz import *
...
class TestHelmholtz(unittest.TestCase):
    ...
    def test_superposition(self):
        radius=0.125
        mesh_resolution=101
        mesh=generate_mesh_with_cicular_subdomain(mesh_resolution,radius,False)
        ...
        kappa = dl.Expression('((x[0]-c1)*(x[0]-c1)+(x[1]-c1)*(x[1]-c1) >= r*r + tol) ? k_0 : ((x[0]-c2)*(x[0]-c2)+(x[1]-c2)*(x[1]-c2)>=r*r/4+tol ? k_1 : k_2)', degree=0, tol=1e-14, k_0=kappas[0], k_1=kappas[1],k_2=kappas[2],r=radius,c1=cr1,c2=cr2)
        ...
        def get_boundary_conditions():
            boundary_conditions=[['neumann',bndry_obj[ii],[dl.Constant(0),beta]]
                             for ii in [1,4,7,10]]
            boundary_conditions+=[
                ['robin',bndry_obj[ii],
