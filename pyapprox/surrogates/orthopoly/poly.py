...
from pyapprox.surrogates.orthopoly.orthonormal_recursions import (
    ...
    hahn_recurrence, charlier_recurrence, discrete_chebyshev_recurrence,
    laguerre_recurrence)
...
class OrthonormalPolynomial1D(ABC):
    ...
    def __eq__(self, other):
        return (self.__class__.__name__ == self.__class__.__name__ and
                ...
                self._rcoefs == other._self._rcoefs)
    ...
    def derivatives(self, samples, nmax, order, return_all=False):
        ...
        for _order in range(1, order+1):
            derivs = self._bkd._la_full((nsamples, nindices), 0.)
            for jj in range(_order, nindices):
                if (jj == _order):
                    # use following expression to avoid overflow issues when
                    # computing oveflow
                    derivs[:, jj] = self._bkd._la_exp(
                        gammaln(_order+1)-0.5*self._bkd._la_sum(
                            self._bkd._la_log(b[:jj+1]**2)))
                else:

                    derivs[:, jj] = (
                        (samples-a[jj-1]) * derivs[:, jj-1]-b[jj-1] *
                        derivs[:, jj-2] + _order*vals[:, jj-1])
                    derivs[:, jj] *= 1.0/b[jj]
            vals = derivs
    ...
    def gauss_quadrature_rule(self, npoints):
        ...
        if npoints > self._ncoefs():
            raise ValueError("{0}: Too many terms requested. {1}".format(
                self, "npoints={0} > ncoefs={1}".format(
                    npoints, self._ncoefs())))
...
class HermitePolynomial1D(OrthonormalPolynomial1D):
    def __init__(self, rho=0., prob_meas=True, backend=None):
