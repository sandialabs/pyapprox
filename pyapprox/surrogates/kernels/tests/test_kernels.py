...
class TestKernels:
    ...
    def test_kernels(self):
        ...
        Y = bkd.array(np.random.normal(0, 1, (2, nsamples2)))
        assert bkd.allclose(
            kernel_inf.diag(X), bkd.get_diagonal(kernel_inf(X, X))
        )

        const0 = 2.0
        kernel_prod = kernel_inf * ConstantKernel(const0, backend=bkd)
        assert bkd.allclose(kernel_prod.diag(X), const0 * kernel_inf.diag(X))
        assert bkd.allclose(
            kernel_prod.diag(X), bkd.get_diagonal(kernel_prod(X, X))
        )
        assert bkd.allclose(kernel_prod(X, Y), const0 * kernel_inf(X, Y))
