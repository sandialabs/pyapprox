...
def precompute_multivariate_orthonormal_polynomial_univariate_values(
    ...
    if basis_type_index_map is None:
        ...
        basis_type_index_map = np.zeros((nvars), dtype=np.int_)
        recursion_coeffs = [recursion_coeffs]

    for dd in range(nvars):
        if (
            recursion_coeffs[basis_type_index_map[dd]].shape[0]
            <= max_level_1d[dd]
        ):
    ...
    basis_vals_1d = np.empty(
        (
            nvars,
            (1 + deriv_order) * (max_level_1d.max() + 1),
            samples.shape[1],
        ),
        dtype=np.float64,
    )
    ...
    for dd in range(nvars):
        basis_vals_1d[dd, : (deriv_order + 1) * (max_level_1d[dd] + 1), :] = (
            evaluate_orthonormal_polynomial_deriv_1d(
                samples[dd, :],
                max_level_1d[dd],
                recursion_coeffs[basis_type_index_map[dd]],
                deriv_order,
            ).T
        )
...
def evaluate_multivariate_orthonormal_polynomial_values(
    ...
    temp1 = basis_vals_1d.reshape(
        (nvars * basis_vals_1d.shape[1], num_samples)
    )
    temp2 = temp1[
        indices.ravel()
        + np.repeat(np.arange(nvars) * basis_vals_1d.shape[1], num_indices),
        :,
    ].reshape(nvars, num_indices, num_samples)
...
def evaluate_multivariate_orthonormal_polynomial_derivs(
    ...
    temp3 = temp1[
        indices.ravel()
        + np.repeat(
            max_level_1d + 1 + np.arange(nvars) * basis_vals_1d.shape[1],
            num_indices,
        ),
        :,
    ].reshape(nvars, num_indices, num_samples)

    derivs = np.empty((num_samples * nvars, num_indices))
    for jj in range(nvars):
        derivs[(jj) * num_samples : (jj + 1) * num_samples] = (
            np.prod(temp2[:jj], axis=0)
            * np.prod(temp2[jj + 1 :], axis=0)
            * temp3[jj]
        ).T
...
def evaluate_multivariate_orthonormal_polynomial(
    ...
    indices : np.ndarray (nvars, num_indices)
...
class PolynomialChaosExpansion(object):
    ...
    def update_recursion_coefficients(self, num_coefs_per_var):
        ...
        for key, poly_opts in self.config_opts["poly_types"].items():
            if initializing or (
                np.any(
                    num_coefs_per_var[self.basis_type_var_indices[ii]]
                ...
                num_coefs = num_coefs_per_var[
                    self.basis_type_var_indices[ii]
                ].max()
            ...
            ii += 1
        if (
            np.unique(np.hstack(self.basis_type_var_indices)).shape[0]
            != self.nvars()
        ):
    ...
    def jacobian(self, sample):
        ...
        derivative_matrix = self.basis_matrix(sample, {"deriv_order": 1})[1:]
...
def get_univariate_quadrature_rules_from_pce(pce, degrees):
    ...
    if degrees.shape[0] == 1 and nvars > 1:
        degrees = np.array([degrees[0]] * nvars)
...
def get_tensor_product_quadrature_rule_from_pce(pce, degrees):
    ...
    canonical_samples, weights = get_tensor_product_quadrature_rule(
        degrees + 1, pce.nvars(), univariate_quadrature_rules
    )
    samples = pce.var_trans.map_from_canonical(canonical_samples)
...
def marginalize_polynomial_chaos_expansion(poly, inactive_idx, center=True):
    ...
    if poly.indices is not None:
        ...
        for ii, index in enumerate(poly.indices.T):
            if (
                (index.sum() == 0 and center is False)
                or np.any(index[active_idx])
                and (not np.any(index[inactive_idx] > 0))
            ):
...
def compute_univariate_orthonormal_basis_products(
...
def compute_product_coeffs_1d_for_each_variable(
    ...
    def get_recursion_coefficients(N, dd):
        poly.update_recursion_coefficients([N] * nvars)
...
def compute_multivariate_orthonormal_basis_product(
    ...
    active_vars = backend.where(poly_index > 0)[0]
    if active_vars.shape[0] > 0:
        ...
        indices_1d = [
            backend.arange(poly_index[dd] + 1, dtype=int) for dd in active_vars
        ]
        product_coefs = backend.outer_product(coefs_1d)[:, None]
        active_product_indices = backend.cartesian_product(indices_1d)
        II = backend.where(backend.abs(product_coefs) > tol)[0]
        active_product_indices = active_product_indices[:, II]
        product_coefs = product_coefs[II]
        product_indices = backend.full(
            (nvars, active_product_indices.shape[1]), 0.0, dtype=int
        )
        # product_indices[active_vars] = active_product_indices
        product_indices = backend.up(
            product_indices, active_vars, active_product_indices
        )
    else:
        product_coefs = backend.full((1, 1), 1.0)
        product_indices = backend.full([nvars, 1], 0.0, dtype=int)
...
def multiply_multivariate_orthonormal_polynomial_expansions(
    ...
    max_degrees1 = backend.max(poly_indices1, axis=1)
    max_degrees2 = backend.max(poly_indices2, axis=1)
...
def get_univariate_quadrature_rules_from_variable(
    ...
    if max_nsamples.shape[0] == 1:
        max_nsamples = np.ones(variable.nvars(), dtype=int) * max_nsamples[0]
    if max_nsamples.shape[0] != variable.nvars():
        raise ValueError(
...
def get_coefficients_for_plotting(pce, qoi_idx):
    ...
    degree_breaks = []
    coeff_sorted = []
    degree_indices_set = np.empty((nvars, 0))
    for degree in range(max_degree + 1):
        nterms = nchoosek(nvars + degree, degree)
        if nterms < 1e6:
...
def plot_unsorted_pce_coefficients(
    coeffs,
    indices,
    ax,
    degree_breaks=None,
    ...
    for i in range(len(coeffs)):
        ...
        ms = 10.0 - 2.0 * i
        ax.plot(
            list(range(1, coeffs_i.shape[0] + 1)),
            coeffs_i,
            marker=markers[i % 3],
            fillstyle=fs,
...
def _marginalize_function_1d(
    ...
    sub_variable = IndependentMarginalsVariable(
        [all_variables[jj] for jj in range(variable.nvars()) if ii != jj]
    )
    ...
    indices = np.delete(np.arange(variable.nvars()), ii)
...
def _marginalize_function_nd(
    fun, variable, quad_degrees, sub_indices, sub_samples, qoi=0
):
    assert sub_samples.ndim == 2
    assert sub_indices.shape[0] == sub_samples.shape[0]
    assert quad_degrees.shape[0] == variable.nvars() - sub_indices.shape[0]

    if sub_indices.shape[0] == variable.nvars():
    ...
    sub_variable = IndependentMarginalsVariable(
        [
            all_variables[kk]
            for kk in range(variable.nvars())
            if kk not in sub_indices
        ]
    )
    pce = get_polynomial_from_variable(sub_variable)
    xquad, wquad = get_tensor_product_quadrature_rule_from_pce(
    ...
    samples = np.empty((variable.nvars(), xquad.shape[1]))
    indices = np.delete(np.arange(variable.nvars()), sub_indices)
    samples[indices, :] = xquad
