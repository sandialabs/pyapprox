...
from pyapprox.surrogates.affine.indices.plot import (  # noqa: E402
    _resolve_colors,
    _resolve_labels,
    format_index_axes,
    plot_index_sets,
...
class TestResolveColors:
    ...
    def test_callable(self):
        ...
        assert result == ["blue", "red", "blue", "red"]
...
class TestPlotIndices2D:
    ...
    def test_smoke(self):
        ...
        assert len(rects) == 4
        assert len(texts) == 0  # no labels by default

    def test_with_labels(self):
        rects, texts = plot_indices_2d(self.ax, _INDICES_2D, labels=True)
        assert len(rects) == 4
        assert len(texts) == 4
        assert texts[0].get_text() == "(0,0)"
...
class TestPlotIndices3D:
    ...
    def test_smoke(self):
        ...
        assert voxel_result is not None
        assert len(texts) == 0

    def test_with_labels(self):
        _, texts = plot_indices_3d(self.ax, _INDICES_3D, labels=True)
        assert len(texts) == 4
        assert texts[0].get_text() == "(0,0,0)"
...
class TestFormatIndexAxes:
    ...
    def test_2d_axis_labels(self):
        ...
        assert ax.get_xlabel() == "dim 1"
        assert ax.get_ylabel() == "dim 2"
        plt.close(fig)
    ...
    def test_2d_max_indices_overrides_limits(self):
        ...
        assert ax.get_xlim()[1] == pytest.approx(5.5)
        assert ax.get_ylim()[1] == pytest.approx(3.5)
    ...
    def test_3d_limits(self):
        ...
        assert ax.get_xlim()[1] == pytest.approx(1.5)  # max=1 + pad=0.5
        assert ax.get_zlim()[1] == pytest.approx(1.5)  # max=1 + pad=0.5
        plt.close(fig)
    ...
    def test_3d_view_init(self):
        ...
        assert ax.elev == pytest.approx(20)
        assert ax.azim == pytest.approx(60)
        plt.close(fig)
...
class TestPlotIndexSets:
    def test_selected_only_2d(self):
        fig, ax = plt.subplots()
        result = plot_index_sets(ax, _INDICES_2D, selected_labels=True)
        assert len(result["selected"][0]) == 4
        assert len(result["selected"][1]) == 4
        assert len(result["candidates"][0]) == 0
        plt.close(fig)
    ...
    def test_format_axes_applies_labels(self):
        ...
        assert ax.get_xlabel() == "$k_1$"
        assert ax.get_ylabel() == "$k_2$"
        plt.close(fig)
    ...
    def test_max_indices_fixes_limits(self):
        ...
        assert ax.get_xlim()[1] == pytest.approx(8.5)
        assert ax.get_ylim()[1] == pytest.approx(3.5)
        plt.close(fig)
