...
class TestVariationalGP:
    ...
    def _make_kernel(self, bkd, fixed: bool = True) -> Matern52Kernel:
    ...
    def test_fit_no_optimization(self, bkd) -> None:
    ...
    def test_predict_no_optimization(self, bkd) -> None:
        ...
        assert std.shape == (1, self.n_train)

    def test_call_is_predict(self, bkd) -> None:
    ...
    def test_alpha_and_cholesky_accessible(self, bkd) -> None:
        ...
        chol = gp.cholesky()
        assert chol.factor().shape == (self.n_train, self.n_train)

    # ---- Test 2: Collapse to exact GP ----

    def test_collapse_to_exact_gp(self, bkd) -> None:
        ...
        bkd.assert_allclose(mean_v, mean_e, atol=1e-4, rtol=1e-4)

        std_v = vgp.predict_std(X_test)
        std_e = egp.predict_std(X_test)
        bkd.assert_allclose(std_v, std_e, atol=1e-3, rtol=1e-3)

    # ---- Test 3: Kernel params active ----

    def test_fit_kernel_active(self, bkd) -> None:
    ...
    def test_input_output_transforms(self, bkd) -> None:
    ...
    def test_sparse_quadratic_approximation(self, bkd) -> None:
        ...
        fixed at Sobol points. The Nystrom approximation with 25 inducing
    ...
    def test_hyp_list_combines_all(self, bkd) -> None:
        """hyp_list should combine kernel + mean + inducing params."""
        self._setup_data(bkd)
        gp = self._make_gp(bkd, kernel_fixed=False, inducing_fixed=False)
        hyps = gp.hyp_list()
