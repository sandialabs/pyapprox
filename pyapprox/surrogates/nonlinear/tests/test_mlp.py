...
class TestNeuralNetwork(unittest.TestCase):
    ...
    def check_nn_loss_gradients(self, activation_fun):
        ...
        errors = check_gradients(
            fun, jac, zz, plot=False, disp=disp, rel=True, direction=None
        )
    ...
    def check_nn_input_gradients(self, activation_fun):
        ...
        errors = check_gradients(
            fun, jac, x0, plot=False, disp=disp, rel=True, direction=None
        )
    ...
    def test_fit(self):
        ...
        optimizer_opts = {"method": "L-BFGS-B", "options": {"maxiter": 1000}}
        results = network.fit(
            training_samples, training_vals, x0, verbose=1, opts=optimizer_opts
        )
        ...
        for res in incomp_results:
            ...
            plt.plot(xx, network(xx[None, :])[:, 0], lw=0.5, c="g")
        plt.ylim(
            network_vals.min() - 0.1 * abs(network_vals.min()),
    ...
    def test_multitask_learning_hard_sharing(self):
        ...
        obj = partial(network.objective_function, train_samples, train_vals)
        jac = partial(network.objective_jacobian, train_samples, train_vals)
        parameters = np.random.normal(0, 1, (network.nparams))
