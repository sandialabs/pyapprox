...
def squared_loss_function(approx_values, values):
    ...
    """
    return (
        0.5 * np.sum((approx_values - values) ** 2, axis=0) / values.shape[1]
    )
...
# TODO modify this class to take advantage of all new apis. Only made
# enough changes to get code to run but tests are currently failing
# because they do not take adavantage to check_gradients provided by
# model


class NeuralNetwork(OptimizedRegressor):
    def __init__(self, opts, backend: BackendMixin = NumpyMixin):
    ...
    def parse_options(self):
        ...
        if activation_type not in activation_functions:
            ...
            msg += "supported"
            raise Exception(msg)
        self.afunc, self.agrad = activation_functions[activation_type]
        ...
        self._nvars = self.layers[0]
        self._nqoi = self.layers[-1]
    ...
    def layer_backwards_propgate(
        self, delta_l, Wmats, layer, jacobian, ub, activation=True
    ):
    ...
    def objective_jacobian(self, train_samples, train_values, parameters):
        if self.last_parameters is None or not np.allclose(
            parameters, self.last_parameters, rtol=1e-12
        ):
    ...
    def objective_function(self, train_samples, train_values, parameters):
        assert train_values.ndim == 2
        assert train_values.shape[0] == train_samples.shape[1]
        self.last_parameters = parameters
        approx_values = self.forward_propagate(train_samples, parameters)
    ...
    def fit(self, samples, values, x0=None, opts={}, verbose=0):
        ...
        obj = partial(
            self.objective_function, self.train_samples, self.train_values
        )
        jac = partial(
            self.objective_jacobian, self.train_samples, self.train_values
        )
        if x0 is None:
        ...
        for x in x0.T:
            ...
            if verbose > 0:
                print(
                    f"Repeat {len(results)}/{x0.shape[1]}",
                    "Loss",
                    results[-1].fun,
                    "Success",
                    results[-1].success,
                    "gnorm",
                    np.linalg.norm(results[-1].jac),
                )
        ...
        if verbose > 0:
            print(
                "No. restarts completed",
                np.where(completed_solves)[0].shape[0],
                f"/ {x0.shape[1]}",
            )
            print("Losses")
            print(obj_vals)
...
class MultiTaskNeuralNetwork(NeuralNetwork):
    ...
    def objective_jacobian(
        ...
        for train_samples, train_values in zip(
            ...
            task += 1
        grad += (
            self.lag_mult * parameters.squeeze() / train_samples[0].shape[0]
        )
