...
class TestETC(unittest.TestCase):
    ...
    def test_AETC_optimal_loss(self):
        ''' Tests if the optimal loss returned from using oracle stats is the same as using without oracle stats given many samples.  '''
        alpha = 1000
        nsamples = int(1e6)
        shifts = np.array([1, 2])
        ...
        result_oracle = est_or._optimal_loss(
            target_cost, hf_values, covariate_values, costs, covariate_subset,alpha, exploit_cost)
    ...
    def test_aetc_blue(self):
        ...
        true_beta_Sp = estimator._least_squares(oracle_hf_values,oracle_covariate_values)[0]
