...
def _plot_model_recursion(recursion_index: Array, ax: plt.Axes):
    ...
    nx.draw(
        graph, pos=pos, ax=ax, with_labels=True, node_size=[2000], font_size=24
    )


def _plot_partition(ii: int, jj: int, ax: plt.Axes, color: str, text: str):
    box = np.array(
...
def _autolabel(
    ax: plt.Axes,
    rects: mpl.container.BarContainer,
    model_labels: List[str],
    vert_shifts: List[float] = None,
):
