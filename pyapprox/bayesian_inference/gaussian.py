...
def get_matrix_partition_indices(selected_block_ids,block_ids,
    ...
    """
    nvector_entries,nblocks=np.sum(nentries_per_block),len(nentries_per_block)
    ...
    relevant_block_ids,relevant_block_indices,relevant_selected_block_indices=\
    ...
    selected_vector_indices_reduced = subselect_matrix_blocks(
        relevant_selected_block_indices,nentries_per_block)
...
def get_unique_variable_blocks(var1_ids,nvars_per_var1,var2_ids,nvars_per_var2):
    ...
    for ii in range(len(var2_ids)):
        ...
        if not var_id in var1_ids:
            ...
            nvars_per_all_vars=np.append(nvars_per_all_vars,nvars_per_var2[ii])
...
def condition_gaussian_in_canonical_form(fixed_indices, precision_matrix,
    ...
    new_normalization = normalization+hy.dot(data)-0.5*data.T.dot(KYY.dot(data))
...
def joint_density_from_linear_conditional_relationship(mean1,cov1,cov2g1,
    ...
    Compute normal joint density  p(x1,x2) with mean and covariance
        m=[  m1  ], C=[C1       C1*A^T    ]  
          [A*m1+b]    [A*C1 C_2|1+A*C1*A^T]
    ...
    joint_mean : np.ndarray (nvars2+nvars1)
       The mean (m) of the joint distribution

    joint_cov : np.ndarray (nvars2+nvars1,nvars2+nvars1)
       The covariance C of the joint distribution
    """
...
def convert_conditional_probability_density_to_canonical_form(
    ...
    Parameters
    ----------
    cov : np.ndarray (nvars2,nvars2)
        The covariance (C_2|1) of the Gaussian distribution P(x2|x1)
    ...
    precision_matrix : np.ndarray (nvars2, nvars2)
        The matrix (K) of the Gaussian P(x2|x1)

    shift : np.ndarray (nvars2)
        The vector (h) of the Gaussian P(x2|x1)

    new_normalization : float
        The normalization constant (g) of the Gaussian P(x2|x1)

    all_var_ids : list (nvars)
        List of ids of the variable blocks in P(x2|x1)

    nvars_per_all_vars : list (nvars)
        Sizes of the variable blocks in P(x2|x1)
    """
    Cinv = np.linalg.inv(cov)
    CinvA = Cinv.dot(Amat)
    precision_matrix = np.block([[Amat.T.dot(CinvA),-CinvA.T],[-CinvA,Cinv]])
...
class GaussianFactor(object):
    ...
    TODO: Explain what I mean by compact here...
    """
    ...
    def condition(self,data_ids,data):
        ...
        precision_matrix, shift, normalization = \
            condition_gaussian_in_canonical_form(
                remove_indices, self.precision_matrix, self.shift,
