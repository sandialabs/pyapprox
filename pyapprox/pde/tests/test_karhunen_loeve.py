...
class TestKLE:
    ...
    def test_multivariate_chain_rule(self):
        ...
        def yfun(u):
            return u[0] ** 2 + 2.0 * u[1]

        def ufun(x):
            return np.array([x[0] * np.sin(x[1]), np.sin(x[1]) ** 2])

        sample = np.random.normal(0.0, 1.0, (2))

        exact_gradient = np.array(
            [
                2.0 * sample[0] * np.sin(sample[1]) ** 2,
                (sample[0] ** 2 + 2.0) * np.sin(2.0 * sample[1]),
            ]
        )

        uvec = ufun(sample)
        jac_yu = np.array([2 * uvec[0], 2.0])
        jac_ux = np.array(
            [
                np.array([np.sin(sample[1]), 0.0]),
                np.array(
                    [sample[0] * np.cos(sample[1]), np.sin(2.0 * sample[1])]
                ),
            ]
        ).T
    ...
    def test_compute_kle_gradient_from_mesh_gradient(self):
        ...
        nvars, sigma = 2, 3
        length_scale = 1
        mesh = bkd.linspace(0.0, 1.0, 11)[None, :]
        kle_mean = mesh[0, :] + 2
        ...
        for use_log in [False, True]:
            ...
            sample = bkd.array(np.random.normal(0.0, 1.0, (nvars, 1)))
            kle_vals = kle(sample)

            mesh_gradient = kle_vals.T * 2
            assert np.allclose(
                mesh_gradient,
                approx_jacobian(scalar_function_of_field, kle_vals),
                atol=1e-7,
            )
