...
from pyapprox.pde.spectral_diffusion import (
    ...
    RectangularCollocationMesh, ones_fun_axis_0
)
...
class StokesFlowModel(AbstractSpectralPDE):
    ...
    def _form_collocation_matrix(self):
        ...
        for dd in range(self._mesh.nphys_vars):
            ...
            sub_mats[-1][self._mesh.nphys_vars] = (
                self._pres_mesh.derivative_matrices[dd])
    ...
    def _apply_boundary_conditions_to_matrix(self, matrix):
        ...
        cnt = 0
        for dd in range(self._mesh.nphys_vars):
            vel_matrix = matrix[cnt:cnt+nvel_dof, cnt:cnt+nvel_dof]
            ...
            vel_matrix = self._mesh._apply_boundary_conditions_to_matrix(
                vel_matrix)
            matrix[cnt:cnt+nvel_dof, cnt:cnt+nvel_dof] = vel_matrix
            cnt += nvel_dof
        return matrix
    ...
    def solve(self, sample):
        ...
        nvel_dof = self._mesh.mesh_pts.shape[1]
        split_solutions = [solution[ii*nvel_dof:(ii+1)*nvel_dof]
                           for ii in range(self._mesh.nphys_vars)]
        split_solutions.append(solution[self._mesh.nphys_vars*nvel_dof:])
...
class TestStokes(unittest.TestCase):
    ...
    def test_stokes_2d(self):
        ...
        sp_forc = [-(vel.diff(s, 2)-sp_pres.diff(s, 1)) for vel, s in zip(sp_vel, [sp_x, sp_y])]
        print(sp_forc)
        div = sum([vel.diff(s, 1) for vel, s in zip(sp_vel, [sp_x, sp_y])])
        assert (div == 0)
        exact_vel_lambda = [sp.lambdify((sp_x, sp_y), fun, "numpy")
                            for fun in sp_vel]
        exact_pres_lambda = sp.lambdify((sp_x, sp_y), sp_pres, "numpy")
        def exact_vel(x):
            vals = [fun(x[0], x[1])[:, None] for fun in exact_vel_lambda]
            return vals
        def exact_pres(x):
            vals = exact_pres_lambda(x[0], x[1])[:, None]
            return vals
        ...
        forc_fun_np = sp.lambdify((sp_x, sp_y), sp_forc, "numpy")
        def forcing_fun(x, z):
            vals = forc_fun_np(x[0], x[1]) # vals is a list
