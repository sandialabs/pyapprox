...
class TestMixtureModel(unittest.TestCase):
    def test_mixture_model_sparse_grid_quadrature(self):
        ...
        for ii in range(num_mixtures):
            lambda_sampler=lambda a,b,nn:2*np.random.beta(a,b,(num_vars,nn))-1
            # partial is needed to make sure correct alpha and beta parameters
            # are used and not overwritten
            sampler = partial(lambda_sampler,rv_params[ii][0],rv_params[ii][1])
        ...
        sg_samples, sg_weights = get_mixture_sparse_grid_quadrature_rule(
            mixture_univariate_quadrature_rules,mixture_univariate_growth_rules,
            num_vars,level)
    ...
    def test_compute_grammian_of_mixture_models_using_sparse_grid_quadrature(self):
        num_vars = 2; degree = 3
        #rv_params = [[6,2],[2,6]]
        rv_params = [[1,1]]
        leja_basename = None
        ...
        grammian_matrix = \
            compute_grammian_of_mixture_models_using_sparse_grid_quadrature(
                poly.basis_matrix,indices,mixture_univariate_quadrature_rules,
                mixture_univariate_growth_rules,num_vars)

        assert (np.all(np.isfinite(grammian_matrix)))

        if num_mixtures==1:
            assert poly_opts['alpha_poly']==rv_params[0][1]-1
            assert poly_opts['beta_poly']==rv_params[0][0]-1
            I = np.where(abs(grammian_matrix)>1e-8)
