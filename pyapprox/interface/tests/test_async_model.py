...
def check_model_values(
    ...
    """
    samples = np.random.uniform(-1.0, 1.0, (num_vars, num_samples))
    vals = model(samples)
...
def check_fileiomodel_files(directories, link_filenames):
    for directory in directories:
        ...
        # results.out.i
        dir_num = int(re.findall(r"[0-9]+", os.path.split(directory)[1])[-1])
        assert os.path.exists(
            os.path.join(directory, "params.in.%d" % dir_num)
        )
        assert os.path.exists(
            os.path.join(directory, "results.out.%d" % dir_num)
        )
...
def get_file_io_model(delay=0.0, fault_percentage=0):
    """
...
class TestAsyncModel(unittest.TestCase):
    ...
    def test_fault_tolerant_async_model_full_save(self):
        ...
        for workdir in workdirs:
            ...
            assert os.path.exists(
                os.path.join(
                    workdir, model.params_filename + ".%d" % function_eval_id
                )
            )
    ...
    def test_async_model_backup(self):
        ...
        saved_data_basename = os.path.join(temp_dirname, saved_data_basename)
