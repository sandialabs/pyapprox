...
nvars = 2
benchmark = GenzBenchmark(name="oscillatory", nvars=nvars)
...
class MeanErrorEstimator(ConvergenceErrorEstimator):
    def _estimate(self, model):
        return model._interp.mean() - benchmark.integral()
...
sols, times = model._fwd_solver.solve(
    ...
    0,
    model._final_time,
    newton_kwargs=model._newton_kwargs,
    verbosity=0,
)
ani = generate_animation(
    model._fwd_solver.physics.mesh,
    sols,
    times,
    filename=None,
    maxn_frames=100,
...
convergence_data = run_convergence_study(
    ...
    benchmark.variable,
    validation_levels,
    benchmark.get_num_degrees_of_freedom,
    benchmark.config_var_trans,
    num_samples=1,
    coarsest_levels=coarsest_levels,
