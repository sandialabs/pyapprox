...
We use Monte Carlo training samples throughout this tutorial. For smooth 1D targets, $K \approx 2P$ is enough to land in a clean training regime. For harder or higher-dimensional targets, we use $K = 5P$. If the loss landscape feels noisy or optimization stalls, increasing the sample ratio or switching to Gauss--Legendre quadrature usually helps.
:::
...
How does the error change as we increase $N$ and the maximum polynomial degree?

```{python}
configs = [(2, 2), (2, 4), (2, 6), (2, 8),
           (4, 2), (4, 4), (4, 6),
           (8, 2), (8, 4)]

sweep_P, sweep_err = [], []

for N_val, L_val in configs:
    np.random.seed(0)
    supn_i = create_supn(nvars=1, width=N_val, max_level=L_val, bkd=bkd)
    P_i = supn_i.nparams()
    K_i = 2 * P_i

    x_tr = np.random.uniform(-1, 1, K_i).reshape(1, -1)
    y_tr = runge(x_tr)

    fitter_i = SUPNMSEFitter(bkd)
    result_i = fitter_i.fit(supn_i, bkd.asarray(x_tr), bkd.asarray(y_tr))
    fitted_i = result_i.surrogate()

    y_p = bkd.to_numpy(fitted_i(bkd.asarray(x_test)))
    err = np.linalg.norm(y_p - y_test) / np.linalg.norm(y_test)

    sweep_P.append(P_i)
    sweep_err.append(err)
    print(f"  (N={N_val}, L={L_val})  P={P_i:3d}  rel L2 = {err:.3e}")
```

```{python}
#| fig-cap: "Convergence of SUPN approximations to the Runge function as a function of total parameter count P. The error decays rapidly over the range studied here, characteristic of smooth-function approximation."
#| label: fig-runge-convergence

from pyapprox_tutorials.figures._supn import plot_supn_convergence

fig, ax = plt.subplots(figsize=(6, 4))
plot_supn_convergence(
    sweep_P, sweep_err, ax,
    title="Runge function convergence",
)
plt.tight_layout()
plt.show()
```

Two observations from the sweep. First, for fixed width $N$, increasing the polynomial degree (larger $M$) is what drives the error down --- this is the SUPN inheriting the approximation power of polynomials. Second, increasing $N$ at fixed $M$ helps less, because $N$ is bounded above in usefulness by the expressivity of the inner polynomial. We will see this trade-off more directly in the 2D heat-map below.
...
x_train = np.random.uniform(-1, 1, (2, K))
...
rel_l2 = np.linalg.norm(z_pred - z_true) / np.linalg.norm(z_true)
...
```{python}
#| fig-cap: "Three-panel comparison on the 2D Rastrigin sum (N=8, L=14, P=968, K=4840). Left: target function; middle: SUPN prediction; right: pointwise absolute error."
...
for i, N_val in enumerate(N_values):
    for j, L_val in enumerate(L_values):
        ...
        x_tr = np.random.uniform(-1, 1, (2, K_ij))
        ...
        z_p = bkd.to_numpy(
            fitted_ij(bkd.asarray(samples_grid))
        ).reshape(X_np.shape)
        err_grid[i, j] = (
            np.linalg.norm(z_p - z_true) / np.linalg.norm(z_true)
        )
```
...
- Start with $K \approx 5P$ Monte Carlo samples for oscillatory or higher-dimensional targets. For smooth 1D targets, $K \approx 2P$ often suffices.
...
1. **Width vs degree trade-off.** Re-run the heat-map with $K = 4P$ training samples instead of $2P$. Does the optimal $(N, \texttt{max\_level})$ pair change, or just the absolute error level?
