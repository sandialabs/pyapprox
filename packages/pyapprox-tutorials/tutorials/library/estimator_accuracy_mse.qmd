...
model = bm.problem().models()[0]       # highest-fidelity model
variable = bm.problem().prior()
K = model.nqoi()              # 3 QoIs

# --- True moments from the benchmark (high-accuracy quadrature) ---
# Statistics methods are directly on the benchmark (inherits mixin)
true_mean = bkd.to_numpy(bm.means())[0]          # (K,) model 0 means
cov_full = bkd.to_numpy(bm.covariance_matrix())
...
W_true = bkd.to_numpy(
    bm.covariance_of_centered_values_kronecker_product_subproblem(
        [0], list(range(K))
    )
)
...
B_true = bkd.to_numpy(
    bm.covariance_of_mean_and_variance_estimators_subproblem(
        [0], list(range(K))
    )
)
```
