...
from pyapprox_tutorials.figures import plot_duu_beam_schematic
...
`ActiveSetFunction` fixes the non-design variables at nominal values and exposes only $w$ and $t$ to the optimizer. It propagates the Jacobian from the wrapped model's `Derivatives` bundle, so the optimizer can use gradient information.
...
class DetConstraintWrapper:
    ...
    def derivatives(self):
        # Forward the wrapped function's derivative bundle (same inputs)
        return self._asf.derivatives()
...
from pyapprox_tutorials.figures import plot_duu_comparison
