...
description: "How to drive the sample-allocation optimizer for group ACV estimators, and how to tell whether the allocation it returned is trustworthy. The allocation objective is small enough that an absolute solver tolerance can report convergence without moving, so solver settings are part of the problem specification rather than defaults to leave alone. The recipe is SLSQP with log-space variable scaling. The MLBLUE convex SDP reformulation is sound, but which cvxpy backend solves it decides whether you receive the optimum. Mean-guided pruning rescues variance estimation at tight budgets."
tutorial_type: usage
...
One caveat about reading this figure at all: it is drawn at `ftol=1e-10`. At
the SciPy default `ftol=1e-6` the same sweep looks entirely different and is
not trustworthy — `none` and `cost` both stall at the initial guess by
$P = 5000$, landing on the Monte Carlo reference and inflating log scaling's
apparent margin to roughly $110\times$. That is the silent stall of
@sec-silent-stall reappearing as a plausible-looking figure, which is why
tolerance comes first in this tutorial and scaling second.


## The MLBLUE Convex Reformulation, and Which Solver Can Follow It
...
Every backend eventually meets this. They differ by orders of magnitude in
*when*, and that difference is what @fig-spd-vs-slsqp is about.
...
```{python}
#| echo: false
#| fig-cap: "MLBLUE mean estimation on the five-model polynomial benchmark: estimator variance against target budget for SLSQP+log/ineq (blue, solid, circles) and for the SDP allocator solved through two cvxpy backends — CVXOPT (green, dash-dot, squares) and CLARABEL (red, dashed, stars). Shading marks the CLARABEL suboptimality, annotated where the gap exceeds $1.2\\times$. Ratios are tabulated in @tbl-spd-backends."
...
problems. A convexity guarantee is a statement about exact arithmetic: it
tells you the problem *has* a unique global optimum, not that your solver
will find it. And a solver reporting convergence is answering a question
about its own internal residual, not about your allocation — which is the
same lesson as @sec-silent-stall, arriving from the opposite direction.
...
- For mean estimation with MLBLUE-style independent subsets the allocation
  problem is convex, and the SDP reformulation
  (`MLBLUESPDAllocationOptimizer`) is a sound way to state it — but the
  Schur-complement encoding puts the $\Psi$ block and the variance scalar $t$
  on scales that separate as the budget grows, and each backend loses the
  optimum once its conditioning tolerance is exceeded. CVXOPT holds the
  gradient solver's allocation to within 7% out to $P = 500$ and drifts to
  $8.7\times$ by $P = 20000$; CLARABEL, cvxpy's default, is already
  $13.7\times$ off at $P = 50$ (see @tbl-spd-backends). Use SLSQP with
  `log/ineq` as the workhorse, and name `solver_name="CVXOPT"` if you want
  the SDP as a cross-check.
