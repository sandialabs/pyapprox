...
from pyapprox_tutorials.figures._sparse_grids import plot_model_hierarchy
...
from pyapprox_tutorials.figures._sparse_grids import plot_config_vars
...
def make_cosine_model(eps):
    """Return a FunctionProtocol model: samples (1, n) -> values (1, n)."""
    def _fn(samples):
        return bkd.reshape(
            bkd.cos(np.pi * (samples[0] + 1) / 2 + eps), (1, -1)
        )
    # Wrap the plain callable so downstream wrappers (e.g.
    # TimedModelFactory) can introspect it as a FunctionProtocol.
    return FunctionFromCallable(nvars=1, nqoi=1, fun=_fn, bkd=bkd)
...
from pyapprox_tutorials.figures._sparse_grids import plot_mf_indices_1d
...
from pyapprox_tutorials.figures._sparse_grids import plot_mf_animation
...
from pyapprox_tutorials.figures._sparse_grids import plot_mf_manual_convergence
...
By default, all models are treated as having equal cost (`ConstantCostModel`).  For realistic multi-fidelity problems, the high-fidelity model is far more expensive.  `ExponentialConfigCostModel` assigns cost $= \text{base}^{\sum \alpha_i}$.  All error indicators divide their priority by `subspace_cost` internally, so swapping in a non-trivial `cost_model` is enough to make the algorithm prefer refining cheap models first --- no separate "cost-weighted wrapper" is needed.
...
# Unit-cost baseline (ConstantCostModel; cumulative cost computed with
...
# Fidelity-weighted run: the default indicator already divides priority
# by subspace_cost, so passing ExponentialConfigCostModel is sufficient.
...
from pyapprox_tutorials.figures._sparse_grids import plot_cost_weighted_animation
...
The cost-weighted algorithm delays promoting $\alpha=1$ candidates because their priority is divided by `subspace_cost`, which for high-fidelity indices includes the 4$\times$ model-cost factor from `ExponentialConfigCostModel`.  This means it can build up physical resolution with the cheap model before investing in the expensive correction, reaching the same accuracy at lower total cost.
...
# The default indicator already cost-weights; just pass the cost model.
fitter_mc = MultiFidelityAdaptiveSparseGridFitter(
    bkd, tp_factory_mc, admis_mc, nconfig_vars=1,
    cost_model=measured_cost2,
)
...
from pyapprox_tutorials.figures._sparse_grids import plot_mf_indices_2d
...
from pyapprox_tutorials.figures._sparse_grids import plot_mf_2d_accuracy
...
- **Cost-aware refinement** via a non-trivial `cost_model` (e.g. `ExponentialConfigCostModel`, `MeasuredCostModel`) ensures that cheap models are exploited before expensive ones.  All error indicators divide priority by `subspace_cost` internally, so the cost model alone controls cost-aware behaviour.
