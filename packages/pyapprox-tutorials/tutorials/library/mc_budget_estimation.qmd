...
- Use the PyApprox estimator API (`MultiOutputMeanAndVariance`, `MCEstimator`, `MCAllocator`) to compute the sample budget for a target accuracy
...
4. Create an `MCAllocator` and call `allocate` with a target computational cost to determine the optimal number of samples.
...
np.random.seed(123)
samples_allocated = fitted.generate_samples_per_model(variable.rvs)[0]
values_allocated = model(samples_allocated)

# Compute the statistics
result = fitted(values_allocated)
...
n_reps = 200
M_alloc = int(fitted.nsamples_per_model()[0])
...
cov_true = bm.covariance_matrix()[:nqoi, :nqoi]
W_true = bm.covariance_of_centered_values_kronecker_product_subproblem(
    [0], list(range(nqoi))
)
B_true = bm.covariance_of_mean_and_variance_estimators_subproblem(
    [0], list(range(nqoi))
)
...
fitted_ref = MCAllocator(est_ref).allocate(target_for_test)
true_pred_std = float(bkd.sqrt(fitted_ref.covariance()[0, 0]))
...
for M_p in pilot_sizes:
    ...
    for i in range(n_pilot_reps):
        ...
        fitted_p = MCAllocator(est_p).allocate(target_for_test)
        pred_stds[i] = float(bkd.sqrt(fitted_p.covariance()[0, 0]))
...
fitted_full = MCAllocator(est_full).allocate(target_cost)
print(f"Allocated samples: {fitted_full.nsamples_per_model()}")
print(f"Actual cost: {float(fitted_full.nsamples_per_model()[0]) * cost_per_eval:.1f} "
      f"(target: {target_cost})")

# 5. Generate samples and evaluate model
np.random.seed(1000)
samples_final = fitted_full.generate_samples_per_model(variable.rvs)[0]
values_final = model(samples_final)

# 6. Compute statistics
result_full = fitted_full(values_final)
...
pred_cov_full = fitted_full.covariance()
for k in range(nqoi):
