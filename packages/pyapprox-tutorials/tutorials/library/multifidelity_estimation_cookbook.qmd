...
fitted_cv = CVAllocator(est_cv).allocate(remaining)
print(f"CV predicted std: {float(fitted_cv.covariance()[0,0])**0.5:.6f}")
```
...
fitted_mlmc = FittedACVEstimator(est_mlmc, mlmc_result)

N_alloc = fitted_mlmc.nsamples_per_model()
print(f"MLMC samples per level: {[int(n) for n in N_alloc]}")
print(f"MLMC predicted std: {float(fitted_mlmc.covariance()[0,0])**0.5:.6f}")
```
...
fitted_mfmc = FittedACVEstimator(est_mfmc, mfmc_result)

print(f"MFMC samples per model: {[int(n) for n in fitted_mfmc.nsamples_per_model()]}")
print(f"MFMC predicted std: {float(fitted_mfmc.covariance()[0,0])**0.5:.6f}")
```
...
from pyapprox.statest.groupacv import FittedGroupACVEstimator, MLBLUESPDAllocationOptimizer
...
fitted_mlblue = FittedGroupACVEstimator(est_mlblue, mlblue_result)

print(f"MLBLUE predicted std: {float(fitted_mlblue.covariance()[0,0])**0.5:.6f}")
```
...
best = result_all.best

var_best = float(bkd.to_numpy(best.covariance())[0, 0])
...
best_gmf = result_gmf.best

print(f"Best (GMF+GRD): {best_gmf.__class__.__name__}")
print(f"Predicted std:  {float(bkd.to_numpy(best_gmf.covariance())[0,0])**0.5:.6f}")
```
...
best_shallow = result_shallow.best

print(f"Best (depth≤2): {best_shallow.__class__.__name__}")
print(f"Predicted std:  {float(bkd.to_numpy(best_shallow.covariance())[0,0])**0.5:.6f}")
```
...
print(f"Fixed γ={list(ri_star)}: {result_fixed.best.__class__.__name__}")
print(f"Predicted std: "
      f"{float(bkd.to_numpy(result_fixed.best.covariance())[0,0])**0.5:.6f}")
```
...
cov_moacv = bkd.to_numpy(result_mo.best.covariance())
...
for qi in range(nqoi_mo):
    ...
    var_so = float(bkd.to_numpy(result_qi.best.covariance())[0, 0])
...
best_ens = result_ens.best

print(f"Best subset estimator: {best_ens.__class__.__name__}")
print(f"Predicted std: {float(bkd.to_numpy(best_ens.covariance())[0,0])**0.5:.6f}")
...
print(f"SLSQP result std: "
      f"{float(bkd.to_numpy(result_slsqp.best.covariance())[0,0])**0.5:.6f}")
```
