...
# ---------------------------------------------------------------------------
# Estimator variance from the MLBLUE SDP solved through CVXOPT, on the
# five-model polynomial benchmark for mean estimation. Recorded rather
# than computed: see plot_spd_vs_slsqp for why cvxopt is not a docs
# dependency. Measured with cvxopt 1.3.3 (conda-forge), cvxpy 1.7.2.
#
# These are a snapshot of one cvxopt version. They do not track future
# changes to cvxopt's algorithms or defaults, so a later release could
# make them stale without anything here failing. Regenerate with
# _measure_cvxopt_reference in an environment where cvxopt is installed
# from conda-forge.
_CVXOPT_REFERENCE = {
    50.0: 5.434603e-06,
    100.0: 2.733025e-06,
    ...
    1000.0: 3.564352e-07,
    5000.0: 1.771661e-07,
    20000.0: 1.179021e-07,
}


def _measure_cvxopt_reference(
    budgets=(50.0, 100.0, 500.0, 1000.0, 5000.0, 20000.0),
):
    """Recompute the pinned CVXOPT values in _CVXOPT_REFERENCE.

    Not called during rendering. Run it manually to refresh the pinned
    numbers, and only in an environment where cvxopt came from
    conda-forge -- a PyPI cvxopt alongside PyTorch aborts the process
    when both OpenMP runtimes load.

    Returns
    -------
    dict
        Budget to estimator variance, in the literal form the module
        constant uses.
    """
    _, costs, stat, subsets = _setup_mean_problem()
    return {
        tc: _run_mlblue_spd(stat, costs, subsets, tc, solver_name="CVXOPT")
        for tc in budgets
    }
...
def plot_spd_vs_slsqp(
    ...
    CLARABEL is solved live; CVXOPT is a recorded measurement. CVXOPT is
    not a documentation-build dependency, because its PyPI wheel vendors
    its own OpenMP runtime and loading that alongside the one PyTorch
    vendors aborts the process outright -- no exception, no traceback,
    just a dead interpreter. Requiring it to render the docs would make
    every build carry that risk for one curve. Installing cvxopt from
    conda-forge instead links the environment's shared OpenMP and is
    safe; that is how the pinned numbers below were produced.

    Because they are pinned, the CVXOPT curve is a snapshot: it will not
    move if cvxopt changes its algorithms, and it is not regenerated by
    rendering. Re-measure with ``_measure_cvxopt_reference`` if the
    ...
    ax : matplotlib Axes
    budgets : iterable of float
        Target costs to sweep. CVXOPT values are available only for the
        default sweep; other budgets plot the CVXOPT curve as missing.
    """
    bkd, costs, stat, subsets = _setup_mean_problem()
    ...
    spd_vars = {
        "CLARABEL": [
            _run_mlblue_spd(stat, costs, subsets, tc, solver_name="CLARABEL")
            or np.nan
            for tc in budgets
        ],
        "CVXOPT": [_CVXOPT_REFERENCE.get(tc, np.nan) for tc in budgets],
    }
