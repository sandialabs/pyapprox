...
The reaction is provided via an object satisfying
SymbolicReactionExpressionsProtocol that can be shared between the
manufactured solution and the physics class.
"""
...
@runtime_checkable
class SymbolicReactionExpressionsProtocol(Protocol):
    """Symbolic reaction members the manufactured solution calls.

    Physics-side reaction classes (e.g. collocation's LinearReaction)
    satisfy this structurally; the numeric members they also carry are
    the physics layer's business.
    """

    def sympy_expressions(
        self, u0_expr: sp.Expr, u1_expr: sp.Expr
    ) -> Tuple[sp.Expr, sp.Expr]:
        """Return (R0_expr, R1_expr) evaluated at the solution exprs."""
        ...
...
class ManufacturedTwoSpeciesReactionDiffusion(
    ...
    The reaction term is provided via an object satisfying
    SymbolicReactionExpressionsProtocol, allowing the same reaction
    definition to be used by both the manufactured solution (for forcing
    computation) and the physics class (for residual evaluation).
    ...
    reaction : SymbolicReactionExpressionsProtocol
    ...
    def __init__(
        ...
        if not isinstance(reaction, SymbolicReactionExpressionsProtocol):
            raise TypeError(
                "reaction must satisfy SymbolicReactionExpressionsProtocol "
                f"(sympy_expressions method), got {type(reaction).__name__}"
            )
