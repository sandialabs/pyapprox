...
class TestPCEArithmetic:
    ...
    def _create_pce_pair(self, bkd, nvars, level1, level2, nqoi=1):
        """Create two PCEs with different index sets and random coefficients."""
        pce1 = self._create_pce(bkd, nvars, level1, nqoi)
        pce2 = self._create_pce(bkd, nvars, level2, nqoi)
    ...
    def _random_samples(self, bkd, nvars, nsamples=101):
        return bkd.asarray(
    ...
    def test_add_same_indices(self, bkd):
        """pce1 + pce2 with same index sets."""
        pce1, pce2 = self._create_pce_pair(bkd, 2, 3, 3)
        pce3 = pce1 + pce2
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(
            pce3(samples), pce1(samples) + pce2(samples), rtol=1e-12
        )

    def test_add_different_indices(self, bkd):
        """pce1 + pce2 with different index sets."""
        pce1, pce2 = self._create_pce_pair(bkd, 2, 2, 4)
        pce3 = pce1 + pce2
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(
            pce3(samples), pce1(samples) + pce2(samples), rtol=1e-12
    ...
    def test_add_1d(self, bkd):
        """pce1 + pce2 in 1D."""
        pce1, pce2 = self._create_pce_pair(bkd, 1, 3, 5)
        pce3 = pce1 + pce2
        samples = self._random_samples(bkd, 1)
        bkd.assert_allclose(
            pce3(samples), pce1(samples) + pce2(samples), rtol=1e-12
        )

    def test_add_scalar(self, bkd):
        """pce + constant."""
        pce1, _ = self._create_pce_pair(bkd, 2, 3, 3)
        pce3 = pce1 + 5.0
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(pce3(samples), pce1(samples) + 5.0, rtol=1e-12)

    def test_radd_scalar(self, bkd):
        """constant + pce."""
        pce1, _ = self._create_pce_pair(bkd, 2, 3, 3)
        pce3 = 3 + pce1
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(pce3(samples), 3.0 + pce1(samples), rtol=1e-12)

    def test_add_multi_qoi(self, bkd):
        """pce1 + pce2 with nqoi > 1."""
        pce1, pce2 = self._create_pce_pair(bkd, 2, 3, 3, nqoi=2)
        pce3 = pce1 + pce2
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(
            pce3(samples), pce1(samples) + pce2(samples), rtol=1e-12
    ...
    def test_sub_pce(self, bkd):
        """pce1 - pce2."""
        pce1, pce2 = self._create_pce_pair(bkd, 2, 3, 4)
        pce3 = pce1 - pce2
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(
            pce3(samples), pce1(samples) - pce2(samples), rtol=1e-12
    ...
    def test_sub_scalar(self, bkd):
        """pce - constant."""
        pce1, _ = self._create_pce_pair(bkd, 2, 3, 3)
        pce3 = pce1 - 7.0
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(pce3(samples), pce1(samples) - 7.0, rtol=1e-12)

    def test_rsub_scalar(self, bkd):
        """constant - pce."""
        pce1, _ = self._create_pce_pair(bkd, 2, 3, 3)
        pce3 = 4 - pce1
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(pce3(samples), 4.0 - pce1(samples), rtol=1e-12)

    # ------------------------------------------------------------------
    # Multiplication
    # ------------------------------------------------------------------

    def test_mul_1d(self, bkd):
        """pce1 * pce2 in 1D."""
        pce1, pce2 = self._create_pce_pair(bkd, 1, 3, 3, nqoi=2)
        pce3 = pce1 * pce2
        samples = self._random_samples(bkd, 1)
        bkd.assert_allclose(
    ...
    def test_mul_different_indices(self, bkd):
        """pce1 * pce2 with different index sets."""
        pce1, pce2 = self._create_pce_pair(bkd, 2, 2, 4, nqoi=1)
        pce3 = pce1 * pce2
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(
            pce3(samples), pce1(samples) * pce2(samples), rtol=1e-11
    ...
    def test_mul_scalar(self, bkd):
        """pce * constant."""
        pce1, _ = self._create_pce_pair(bkd, 2, 3, 3)
        pce3 = pce1 * 3.0
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(pce3(samples), pce1(samples) * 3.0, rtol=1e-12)

    def test_rmul_scalar(self, bkd):
        """constant * pce."""
        pce1, _ = self._create_pce_pair(bkd, 2, 3, 3)
        pce3 = 2 * pce1
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(pce3(samples), 2.0 * pce1(samples), rtol=1e-12)

    # ------------------------------------------------------------------
    # Power
    # ------------------------------------------------------------------

    def test_pow_0(self, bkd):
        """pce ** 0 = constant 1."""
        pce1, _ = self._create_pce_pair(bkd, 2, 3, 3)
        pce0 = pce1**0
        samples = self._random_samples(bkd, 2)
        expected = bkd.ones((1, samples.shape[1]))
        bkd.assert_allclose(pce0(samples), expected, rtol=1e-12)

    def test_pow_1(self, bkd):
        """pce ** 1 = pce."""
        pce1, _ = self._create_pce_pair(bkd, 2, 3, 3)
        pce_1 = pce1**1
        samples = self._random_samples(bkd, 2)
        bkd.assert_allclose(pce_1(samples), pce1(samples), rtol=1e-12)

    def test_pow_2(self, bkd):
        """pce ** 2."""
        pce1, _ = self._create_pce_pair(bkd, 1, 3, 3)
        pce2 = pce1**2
        samples = self._random_samples(bkd, 1)
        bkd.assert_allclose(pce2(samples), pce1(samples) ** 2, rtol=1e-12)

    def test_pow_3(self, bkd):
        """pce ** 3."""
        pce1, _ = self._create_pce_pair(bkd, 1, 3, 3)
        pce3 = pce1**3
        samples = self._random_samples(bkd, 1)
        bkd.assert_allclose(pce3(samples), pce1(samples) ** 3, rtol=1e-10)

    # ------------------------------------------------------------------
    # Composite expressions
    # ------------------------------------------------------------------

    def test_composite_linear(self, bkd):
        """2 * pce1 - pce2 * 3 + 1."""
        pce1, pce2 = self._create_pce_pair(bkd, 2, 3, 3, nqoi=2)
    ...
    def test_composite_mul_add(self, bkd):
        """(pce1 + pce2) * pce3."""
        pce1 = self._create_pce(bkd, 1, 2)
        pce2 = self._create_pce(bkd, 1, 3)
        pce3 = self._create_pce(bkd, 1, 2)
    ...
    def test_incompatible_bases_raise_typeerror(self, bkd):
    ...
    def test_different_nvars_raise_valueerror(self, bkd):
        """Different nvars raise ValueError."""
        pce1, _ = self._create_pce_pair(bkd, 1, 3, 3)
        pce2, _ = self._create_pce_pair(bkd, 2, 3, 3)
        with pytest.raises(ValueError):
            pce1 + pce2

    def test_pow_negative_raises_valueerror(self, bkd):
        """Negative power raises ValueError."""
        pce1, _ = self._create_pce_pair(bkd, 1, 3, 3)
        with pytest.raises(ValueError):
            pce1 ** (-1)

    def test_pow_noninteger_raises_typeerror(self, bkd):
    ...
    def test_add_does_not_modify_inputs(self, bkd):
    ...
    def test_mul_does_not_modify_inputs(self, bkd):
        """pce1 * pce2 leaves originals unchanged."""
        pce1, pce2 = self._create_pce_pair(bkd, 1, 3, 3)
        coef1_before = bkd.copy(pce1.get_coefficients())
        coef2_before = bkd.copy(pce2.get_coefficients())
        _ = pce1 * pce2
        bkd.assert_allclose(pce1.get_coefficients(), coef1_before)
...
class TestAutogradPCEArithmetic:
    ...
    def test_autograd_add(self):
        ...
        assert torch.abs(jac).max().item() > 0.0
