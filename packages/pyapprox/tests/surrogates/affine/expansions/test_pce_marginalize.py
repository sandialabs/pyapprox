...
class TestPCEDimensionReducer:

    def _build_3d_pce(self, bkd, nqoi: int = 1):
    ...
    def test_reduce_pce_keep_0(self, bkd) -> None:
        """reduce_pce([0]) should give 1 + x."""
        pce, _ = self._build_3d_pce(bkd)
        reducer = PCEDimensionReducer(pce, bkd)

        sub_pce = reducer.reduce_pce([0])
        assert sub_pce.nvars() == 1
    ...
    def test_reduce_pce_keep_02(self, bkd) -> None:
    ...
    def test_reduce_pce_keep_all(self, bkd) -> None:
    ...
    def test_mean_variance_of_marginalized_pce(self, bkd) -> None:
    ...
    def test_equivalence_with_quadrature(self, bkd) -> None:
        ...
        pce, marginals = self._build_3d_pce(bkd)
        reducer = PCEDimensionReducer(pce, bkd)
    ...
    def test_multi_qoi(self, bkd) -> None:
        ...
        pce, _ = self._build_3d_pce(bkd, nqoi=2)
        reducer = PCEDimensionReducer(pce, bkd)

        assert reducer.nqoi() == 2

        sub_pce = reducer.reduce_pce([0])
        assert sub_pce.nqoi() == 2
        assert sub_pce.nvars() == 1
