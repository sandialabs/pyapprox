...
class GaussLegendreQuadratureScaled:
    ...
    def __init__(self, bkd, lb: float = 0.0, ub: float = 1.0) -> None:
        self._bkd = bkd
        self._lb = lb
        self._ub = ub
        self._scale = ub - lb
    ...
    def __call__(self, npoints: int) -> Tuple:
...
class TestBetaMarginal:
    ...
    def test_rvs_in_bounds(self) -> None:
        ...
        assert self._bkd.all_bool(samples_flat >= 0.0)
        assert self._bkd.all_bool(samples_flat <= 1.0)
...
class TestBetaMarginalParametrized:
    ...
    def _create_dist(self, bkd, alpha: float, beta: float, lb: float, ub: float):
    ...
    def test_bounds_accessors(
        self, bkd, name: str, alpha: float, beta: float, lb: float, ub: float
    ) -> None:
        """Test bounds accessor methods for various configurations."""
        dist, _ = self._create_dist(bkd, alpha, beta, lb, ub)
    ...
    def test_pdf_matches_scipy(
        ...
        dist, scipy_dist = self._create_dist(bkd, alpha, beta, lb, ub)
    ...
    def test_rvs_in_bounds(
        ...
        assert bkd.all_bool(samples_flat >= lb)
        assert bkd.all_bool(samples_flat <= ub)
...
class TestBetaMarginalBoundsValidation:
