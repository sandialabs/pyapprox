...
_ = mf.plot_correlation_matrix(
    mf.covariance_to_correlation(cov, bkd=bkd),
    ax=ax,
    model_names=labels,
    label_fontsize=20,
)
...
target_cost = 30
stat = mf.multioutput_stats["mean"](benchmark.nqoi(), backend=bkd)
...
stat_0 = mf.multioutput_stats["mean"](len(qoi_idx), backend=bkd)
