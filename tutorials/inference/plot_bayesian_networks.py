...
data_cpd_mats = [
    np.random.normal(0, 1, (nsamples[ii], nparams[ii])) for ii in range(nnodes)
]
...
noise = [noise_std[ii] * np.random.normal(0, noise_std[ii], (nsamples[ii], 1))]
values_train = [
    b.dot(c) + s + n
    for b, c, s, n in zip(data_cpd_mats, true_coefs, data_cpd_vecs, noise)
]
...
true_post = DenseMatrixLaplacePosteriorApproximation(
    data_cpd_mats[0],
...
true_post.compute(values_train[0])

assert np.allclose(gauss_post[1], true_post.posterior_covariance())
assert np.allclose(gauss_post[0][:, None], true_post.posterior_mean())
...
noise = [
    noise_std[ii] * np.random.normal(0, noise_std[ii], (nsamples[ii], 1))
    for ii in range(nnodes)
]
values_train = [
    b.dot(c) + s + n
