...
def push_forward_pdf(y):
    return push_forward_kde(y.T)[:, None]
...
p = plt.contourf(
    X,
    Y,
    np.reshape(posterior_prob, X.shape),
...
accepted_samples_idx = np.where(
    ...
    > np.random.uniform(0.0, 1.0, (num_prior_samples, 1))
)[0]
...
def posterior_push_forward_pdf(y):
    return posterior_push_forward_kde(y.T).squeeze()
