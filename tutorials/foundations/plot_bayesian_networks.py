...
#%%
#The sum-product eliminate variable algorithm begins by first conditioning all the network factors on the available data
network.convert_to_compact_factors()

query_labels = [node_labels[2]]
query_labels = node_labels[:nnodes]
print(evidence,evidence_ids)
...
#     network.node_ids,network.node_labels[:nnodes],query_labels,evidence_ids)
# print(eliminate_ids)



#%%
#We then must determine which variables to eliminate from the factors

#%%
#To compute this marginal of the posterior we only ever had to deal with factors involving two variables $\theta_i,\theta_{i+1}$. Thus this procedure is more efficient than forming the joint density and then marginalizing. The computational saving become much larger when more nodes are added to the network.
