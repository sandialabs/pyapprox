...
L = 10
bounds = [0, L]
greens_fun_2d = HeatEquation1DGreensKernel(
    1, [1e-3, 100], 2 * np.pi, nterms=100
)
# Make greens function take 1D inputs by setting :math:`tau=0`
greens_fun = ActiveGreensKernel(greens_fun_2d, [3.0], [0.0])
plot_xx = np.linspace(*bounds, 101)[None, :]
X, Y = np.meshgrid(plot_xx[0], plot_xx[0])
G = greens_fun(plot_xx, plot_xx)
ax = plt.figure().gca()
...
# final time
greens_fun = ActiveGreensKernel(greens_fun_2d, [final_time], [0.0])
plot_xx = np.linspace(*bounds, 101)[None, :]
X, Y = np.meshgrid(plot_xx[0], plot_xx[0])
G = greens_fun(plot_xx, plot_xx)
ax = plt.figure().gca()
