...
def f1(x):
    return np.cos(3 * np.pi * x).T


def f2(x):
    return np.exp(-((x - 0.5) ** 2) / 0.25).T


def f3(x):
    return f1(x) + f2(x) + (2 * x - 1).T
...
ensemble_variables = [
    ...
    for marginals in ensemble_univariate_variables
]
polys, nparams = get_total_degree_polynomials(ensemble_variables, degrees)
...
nsamples = [20, 20, 3]
samples_train = [
    variable.rvs(n) for variable, n in zip(ensemble_variables, nsamples)
]
noise_std = [0.01] * nmodels
noise = [
    noise_std[ii]
    * np.random.normal(0, noise_std[ii], (samples_train[ii].shape[1], 1))
...
basis_matrices = [p.basis()(s) for p, s in zip(polys, samples_train)]
...
laplace = DenseMatrixLaplacePosteriorApproximation(
    basis_mat, prior.mean(), prior.covariance(), noise_cov
)
laplace.compute(values)
post_mean = laplace.posterior_mean()
post_cov = laplace.posterior_covariance()


# %%
# Now let's plot the resulting approximation of the high-fidelity data.
hf_prior_shapes = (
    prior_mean[nparams[:-1].sum() :],
    prior_cov[nparams[:-1].sum() :, nparams[:-1].sum() :],
)
hf_posterior = (
    post_mean[nparams[:-1].sum() :],
...
plot_1d_lvn_approx(
    ...
    [0, 1],
    colors=["k"],
    mean_label=r"$\mathrm{Single\;Fidelity}$",
)
axs.set_xlabel(r"$z$")
...
hf_laplace.compute(values_train[-1])
shf_post_mean = hf_laplace.posterior_mean()
shf_post_cov = hf_laplace.posterior_covariance()
assert np.allclose(shf_post_mean, hf_posterior[0])
assert np.allclose(shf_post_cov, hf_posterior[1])
...
rows = [
    np.hstack([s11 * I1, 0 * I1, a31 * s11 * I1]),
...
def f3(x):
    return f2(x) + np.cos(3 * np.pi * x).T


functions = [f1, f2, f3]

ensemble_univariate_variables = [[stats.uniform(0, 1)] * 2] + [
    [stats.uniform(0, 1)]
] * 2
ensemble_variables = [
    IndependentMarginalsVariable(marginals)
...
degrees = [3, 5, 5]
polys, nparams = get_total_degree_polynomials(ensemble_variables, degrees)
basis_matrix_funcs = [p.basis() for p in polys]
...
factor_prior = cond_prob_variable_elimination(network, labels)
...
data_cpd_mats = [b(s) for b, s in zip(basis_matrix_funcs, samples_train)]
