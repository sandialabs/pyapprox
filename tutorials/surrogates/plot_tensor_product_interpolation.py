...
def fun(z):
    return (np.cos(2 * np.pi * z[0, :]) * np.cos(2 * np.pi * z[1, :]))[
        :, np.newaxis
    ]
...
axs.plot(nodes[0, :], nodes[1, :], "o", color=marker_color, ms=10, alpha=alpha)
...
cset = axs.contourf(X, Y, Z, levels=levels, cmap="coolwarm", alpha=alpha)
...
quad_rule = TensorProductQuadratureRule(
    2, [ClenshawCurtisQuadratureRule(bounds=[-1, 1])] * 2
)
...
# Load a benchmark
nvars = 2
benchmark = GenzBenchmark("oscillatory", nvars)
...
for level in range(1, 7):  # nvars = 2
    ...
    ltp = build_lagrange_tp(level, benchmark.model())
    ...
    ptp = build_piecewise_tp(level, benchmark.model())
...
for level in range(1, 7):  # nvars = 2
    ...
    lvalues = ltp.mean()
    lerror = np.linalg.norm(benchmark.integral() - lvalues) / np.linalg.norm(
        benchmark.integral()
    )
    ...
    ptp = build_piecewise_tp(level, benchmark.model())
    pvalues = ptp.mean()
    perror = np.linalg.norm(benchmark.integral() - pvalues) / np.linalg.norm(
        benchmark.integral()
    )
    piecewise_data.append([ptp._basis.nterms(), perror])
lagrange_data = np.array(lagrange_data).T
...
alpha_stats = np.arange(1.0, 12.0, 2.0)
...
for alpha_stat in alpha_stats:
    ...
    ax.semilogy(
        ntrain_samples_list, results, label="{0:1.2f}".format(density_ratio)
    )
