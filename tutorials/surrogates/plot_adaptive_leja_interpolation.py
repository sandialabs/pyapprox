...
def compute_l2_error(
    validation_samples, validation_values, pce, relative=True
):
...
def callback(pce):
    ...
    errors.append(error)
    nsamples.append(pce.ntrain_samples())
...
init_degree = 1
gen = ExpandingMarginGenerator(basis.nvars(), init_degree, 1.0, 10)
basis.set_indices(gen.get_indices())

bexp = AdaptivePolynomialChaosExpansion(
    basis, variable, gen, max_nsamples=200, nqoi=1
)
while bexp.step(benchmark.model()):
    callback(bexp)
...
gen.plot_indices(axs[0])
axs[1].plot(*bexp.get_train_samples(), "o")
axs[2].loglog(nsamples, errors, "o-")
plt.show()
