...
# Extract diagonal params (standardized, then transform to physical)
diag_means = []
diag_stds = []
...
# Extract full-covariance params (standardized, then transform)
...
for ax, title, neg_elbo in [
    (ax1, f"Diagonal Gaussian (neg ELBO = {result_diag.neg_elbo():.2f})",
     result_diag.neg_elbo()),
    (ax2, f"Full-covariance Gaussian (neg ELBO = {result_full.neg_elbo():.2f})",
     result_full.neg_elbo()),
]:
    ax.contourf(E1_m, E2_m, p_grid, levels=10, cmap="Oranges", alpha=0.4)
...
exact_pdf = beta_dist.pdf(p_grid, exact_posterior_alpha, exact_posterior_beta)
...
ax.plot(p_grid, exact_pdf, color="#E67E22", lw=2.5,
        label=f"Exact: Beta({exact_posterior_alpha:.0f}, {exact_posterior_beta:.0f})")
