...
for i, (xpos, label, c) in enumerate([
    ...
    ax1.text(xpos, 1.9, "optimize", ha="center", fontsize=8, color="gray")
    ...
    ax1.add_patch(box2)
    ax1.text(xpos, 1.0, f"$q_{i+1}(\\theta;\\,\\mu_{i+1},\\sigma_{i+1})$",
...
for i, (xpos, label, c) in enumerate([
    ...
    ax2.annotate("", xy=(6.0, 1.7), xytext=(xpos, 2.1),
                 arrowprops=dict(arrowstyle="-|>", lw=1.2, color=c, alpha=0.6))
...
for k, ax in enumerate(axes):
    ...
    ax.plot(theta_grid, vi_pdf, color="#2C7FB8", lw=2.5, ls="--",
            label="Amortized VI")
    ax.axvline(theta_true[k], color="#27AE60", ls="--", lw=1.5, alpha=0.7)
    ax.set_xlabel(r"$\theta$", fontsize=12)
    ax.set_title(f"Group {k} (label = {labels_np[k]:.2f})", fontsize=11)
...
ax1_twin = ax1.twinx()
ln1, = ax1.plot(x_eval, mu_curve, color="#E67E22", lw=2.5, label=r"$\mu(x)$")
ln2, = ax1_twin.plot(x_eval, sig_curve, color="#8E44AD", lw=2.5, ls="--",
                      label=r"$\sigma(x)$")
...
# Test labels (reuse stored data for consistency)
for t in range(K_test):
    label_test_val = test_labels_list[t]
    if abs(label_test_val) <= 1.0:
        ...
        ax1_twin.plot(label_test_val, sig_t, "*", ms=14, color="#8E44AD",
                      markeredgecolor="k", lw=1, zorder=5)
...
# Right: example test posterior (first test group)
...
ax2.plot(theta_grid, exact_pdf, color="#E67E22", lw=2.5, label="Exact conjugate")
