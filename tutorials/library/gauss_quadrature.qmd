...
print(f"Nodes shape: {nodes.shape},  Weights shape: {weights.shape}")
print(f"Nodes:   {bkd.to_numpy(nodes[0])}")
print(f"Weights: {bkd.to_numpy(weights[:, 0])}")
```
...
print(f"Weights sum to : {float(bkd.sum(weights)):.15f}  (probability measure)")
print(f"Exact E[f]     : {exact_expectation:.15f}")
print(f"M={M} Gauss rule: {approx:.15f}")
print(f"Error          : {abs(approx - exact_expectation):.2e}   (machine precision)")
```
...
labels = ["$1/(1+16x^2)$ (analytic)", "$|x-0.3|^{1.5}$ (rough)"]
for ax, ge, te, title in zip(
    ...
    ax.semilogy(Ms, ge, "o-", color="#2C7FB8", label="Gauss-Legendre")
    ax.semilogy(Ms, te, "s-", color="#E67E22", label="PiecewiseLinear (trapezoid)")
...
ax.loglog(Ms, gh_errors, "o-", color="#27AE60", label="Gauss-Hermite")
...
ax.set_title(r"Gauss-Hermite convergence for $\theta \sim \mathcal{N}(0,1)$", fontsize=12)
ax.legend(fontsize=11)
ax.grid(True, which="both", alpha=0.3)
plt.tight_layout()
plt.show()

nd3, wt3 = hermite_rule(3)
nd3_np, wt3_np = bkd.to_numpy(nd3[0]), bkd.to_numpy(wt3[:, 0])
print(f"M=3  Gauss-Hermite estimate: {wt3_np @ f_qoi(nd3_np):.15f}")
print(f"Exact (√2/2 · exp(-1/4))  : {exact_mean:.15f}")
```
...
for ii, M in enumerate([5, 9]):
    ...
    offset_g = ii * 2
    offset_l = ii * 2 + 1
    ax.plot(nd_gauss_np,   np.full(M, offset_g), "o", ms=9, color="#2C7FB8",
            label=f"Gauss $M={M}$" if ii == 0 else None)
    ax.plot(nd_lobatto_np, np.full(M, offset_l), "s", ms=9, color="#E67E22",
            label=f"Gauss-Lobatto $M={M}$" if ii == 0 else None)
    ax.axhline(offset_g, color="#2C7FB8", lw=0.4, alpha=0.5)
    ax.axhline(offset_l, color="#E67E22", lw=0.4, alpha=0.5)
